...
$days = 30;					// Время действия купона в днях. 0 - без ограничения.
//$date_end = 0;			// Завершение действия купона в формате YYYY-mm-dd. 0 - без ограничения.
$uses_total = 1; 			// Сколько раз максимально может использоваться купон. 0 - для бесконечного использования.
$uses_customer = 1; 		// Сколько раз максимально может использоваться купон одним клиентом. 0 - для бесконечного использования.
$coupon_product = array();	// Массив с ID привязанных к купону товаров (значения externalItemId). Если пусто, действует на все товары.
$coupon_name = 'Персональная скидка от Пинковского'; // Название купона (необязательно, по умолчанию = "Дисконтная карта (Esputnik) №...")
$coupon_prefix = 'rs-';		// Префикс купона (необязательно)
...
if ($result){
	echo 'Создан купон с кодом: '.$result;
} else {
	echo 'Ошибка';
}
?>