...
class ControllerModulesupermenu extends Controller{
	...
	protected function index($setting){
		$cacheHash = md5(serialize($setting));
		$cacheName = DIR_CACHE.'cache.supermenu.'.$cacheHash.'.0';
		if (is_file($cacheName) && (filemtime($cacheName) > time() - self::CACHELIFE)){
			$this->output = unserialize(file_get_contents($cacheName));
			return false;
		}
		...
		$this->render();
		file_put_contents($cacheName, serialize($this->output));
