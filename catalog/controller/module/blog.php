...
if (!class_exists('ControllerModuleBlog')) {
	class ControllerModuleBlog extends Controller
	{
            ...
            if (isset($this->data['settings_general']['avatar_widget_status']) && $this->data['settings_general']['avatar_widget_status']) {
            	$this->data = $this->avatar_customer($this->data);
					...
					if (isset($this->data['settings_general']['cache_widgets']) && $this->data['settings_general']['cache_widgets']) {
						$hasha             = md5(serialize($this->data['settingswidget']) . serialize($layout_id) . serialize($this->data['userLogged']) . serialize($this->config->get('config_language_id')) . serialize($this->config->get('config_store_id')));
						$hash_cache        = md5(serialize($this->data['settingswidget']) . serialize($layout_id) . serialize($this->data['userLogged']) . serialize($this->config->get('config_language_id')) . serialize($this->config->get('config_store_id')) . serialize($this->request->get) . serialize($this->request->post) . $this->data['customer_id']);
		...
		public function ColorboxLoader($theme, $data)
		{
			...
			if (file_exists($css_dir . $css_name)) {
				...
				$this->document->addStyle($css_file);
						...
						if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/agootemplates/stylesheet/seocms.css.tpl')) {
							$this->template = $this->config->get('config_template') . '/template/agootemplates/stylesheet/seocms.css.tpl';
						} else {
							$this->template = 'default/template/agootemplates/stylesheet/seocms.css.tpl';
						}
		...
		private function getBlogsReviews($settingswidget, $type = 'reviews')
		{
			...
			$this->data['settings']      = $settingswidget;
			$hash                        = md5(serialize($settingswidget));
