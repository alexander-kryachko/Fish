...
class ControllerModuleOCFilter extends Controller {
  ...
  private function construct($settings = array()) {
    ...
    if ($this->path) {
      ...
      if (isset($this->request->get['sort'])) {
        $url .= ($gi ? '&' : '?').'sort=' . (string)$this->request->get['sort'];
		$gi = true;
      }

      if (isset($this->request->get['order'])) {
        $url .= ($gi ? '&' : '?').'order=' . (string)$this->request->get['order'];
		$gi = true;
      }

      if (isset($this->request->get['limit'])) {
        $url .= ($gi ? '&' : '?').'limit=' . (int)$this->request->get['limit'];
		$gi = true;
      }
  ...
  private function getSelectedOptions() {
	...
	$gi = strpos($this->link, '?') > 0 ? true : false; 
		...
		if ($this->ocfilter['price_type'] == 'links' || $this->ocfilter['price_type'] == 'links-slide') {
			...
			for ($i = 0; $i < $links; $i++) {
				...
				$price_links[] = array(
					'from' 			=> $from,
					'to' 				=> $to,
					'href' 			=> $this->link . ($gi ? '&' : '?') . $this->ocfilter['index'] . '=' . $params,
