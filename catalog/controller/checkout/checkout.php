...
/////////products data
		...
		if(in_array(substr(VERSION, 0, 5), array('1.5.3', '1.5.5'))) {
...
//unset($this->session->data['shipping_methods']);
		...
		$this->language->load('checkout/checkout');
					...
					$this->data['entry_warehouses'] = $this->language->get('entry_warehouses');
...
////////do checkout
		if(($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if(isset($this->request->post['street'])){
				$data['payment_address_1'] = $this->request->post['street'].', '.$this->request->post['house'].' д. '.$this->request->post['flat'];
			}
			...
			$data['shipping_zone'] = $this->request->post['postcode'];
			...
			$fromStep = $this->request->post['step'];
			$toStep = $this->request->get['from'];
			$this->load->model('checkout/order');
			...
			switch($fromStep){
				case 1:
					if (empty($this->session->data['order_id'])){
						...
						//$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('cod_order_status_id'), false, false, 'skip');
						
					} else {
						// обновить контактные данные
						$this->model_checkout_order->updateContacts($this->session->data['order_id'], $data, true);
					}
					break;
				case 2:
					//обновить доставку

					$this->model_checkout_order->updateContacts($this->session->data['order_id'], $data);
					break;
				case 3:
					//обновить оплату
					$this->model_checkout_order->updateContacts($this->session->data['order_id'], $data);
					
					//отправить письма и перенаправить
					if ($toStep == 4){
						$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('cod_order_status_id'), false, false);
						$json['redirect'] = 'http://'.$_SERVER['HTTP_HOST'].'/index.php?route=checkout/success';
					}
					break;
			}
...
////////language block
		...
		if (!empty($this->session->data['order_id'])){
			$order_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order` WHERE `order_id` = ".$this->session->data['order_id']." LIMIT 1");
			if ($order_query->num_rows) {
				$this->data['firstname'] = $order_query->row['firstname'];
				$this->data['lastname'] = $order_query->row['lastname'];
				$this->data['email'] = $order_query->row['email'];
				$this->data['telephone'] = $order_query->row['telephone'];
				$this->data['fax'] = $order_query->row['fax'];
				$this->data['city'] = $order_query->row['payment_city'];
                $this->data['postcode'] = $order_query->row['payment_postcode'];
			}
		}
		...
		$cityAdsProductIds = $cityAdsProductQty = array();
		foreach ($this->cart->getProducts() as $product){
			$cityAdsProductIds[] = $product['product_id'];
			$cityAdsProductQty[] = $product['quantity'];
		}
		
		$_SESSION['cityadsTPM'] = "var xcnt_basket_products = '".implode(',', $cityAdsProductIds)."';".PHP_EOL; // id товаров в корзине
		$_SESSION['cityadsTPM'] .= "var xcnt_basket_quantity = '".implode(',', $cityAdsProductQty)."';".PHP_EOL; // кол-во товаров в корзине (опц.)
...
//		}
		...
		if(utf8_strlen($this->request->post['zone_id'])  == '') {
...
*/
		if($this->request->post['hidd-inp'] > 0){
			if ((utf8_strlen($this->request->post['postcode']) < 2) || (utf8_strlen($this->request->post['postcode']) > 10)) {
				$this->error['postcode'] = $this->language->get('error_postcode');
			}
			if ((utf8_strlen($this->request->post['street']) < 2) || (utf8_strlen($this->request->post['street']) > 20)) {
				$this->error['street'] = $this->language->get('error_street');
			}
			if ((utf8_strlen($this->request->post['house']) < 1) || (utf8_strlen($this->request->post['house']) > 5)) {
