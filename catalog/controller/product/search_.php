...
class ControllerProductSearch extends Controller { 	
	public function index() { 
		...
		if (isset($this->request->get['search'])) {
			$search = mb_convert_case($this->request->get['search'], MB_CASE_TITLE, "UTF-8");
		} else {
			$search = '';
		} 
		...
		foreach ($search_words as $i=> $keyword) {
			...
			$results = $this->model_catalog_search->getProducts($data);
			...
			$product_total = $this->model_catalog_search->getFoundProducts(); 
			$product_quantity = $this->model_catalog_search->getProduct($result['quantity']);
			foreach ($results as $result) {
				...
				$this->data['products'][] = array(
					...
					'model' => $result['model'],
					'attribute_groups' => $this->model_catalog_search->getProductAttributes($result['product_id']),
		...
		$this->data['search'] = strtolower($search);
