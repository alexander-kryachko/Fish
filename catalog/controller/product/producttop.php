<?php
class ControllerProductProducttop extends Controller {
    private $error = array();
...
$this->data['stickers'] =  $product_info['statuses']['product_stickers'];      //pr.status
            ...
            $this->data['breadcrumbs'][] = array(
                ...
                'href'      => $this->url->link('product/producttop', $url . '&product_id=' . $this->request->get['product_id']),
                'separator' => $this->language->get('text_separator')
            );
			...
			if (empty($this->data['description']) && $master_id > 0){
            ...
            if ($results) {
                ...
                $this->data['related_product'] = array(
                    ...
                    'href'       => $this->url->link('product/producttop', 'path='. $full_url .'&product_id=' . $result['product_id'])
                );
            ...
            if($is_master && !$pds_allow_buying_series) {
				...
				$this->data['master_link'] = $this->url->link('product/producttop', '&product_id=' . $master_id);
            }
            ...
            foreach ($results as $result)
            {
                ...
                $this->data['pds'][] = array(
                    ...
                    'product_link' => $this->url->link('product/producttop', $url . '&product_id=' . $result['product_id']),
			...
			if ($is_master || empty($this->data['pds'])){
				if($this->model_catalog_product->getCategoryPath($this->request->get['product_id'])!='0'){
					$href = $this->url->link('product/producttop', 'path=' . $this->model_catalog_product->getCategoryPath($this->request->get['product_id']) . '&product_id=' . $this->request->get['product_id']);
					$this->document->addLink($href, 'canonical');
				}else{
					$href = $this->url->link('product/producttop', 'product_id=' . $this->request->get['product_id']);
					$this->document->addLink($href, 'canonical');
				}
			} else {
				/* BOF: Series canonical test */
				$this->document->addLink($this->data['master_link'], 'canonical');
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/producttop.tpl')) {
                $this->template = $this->config->get('config_template') . '/template/product/producttop.tpl';
            } else {
                $this->template = 'default/template/product/producttop.tpl';
            }
            ...
            $this->data['breadcrumbs'][] = array(
                ...
                'href'      => $this->url->link('product/producttop', $url . '&product_id=' . $product_id),
                'separator' => $this->language->get('text_separator')
            );
			...
			*/
			...
			$this->redirect($this->url->link('product/producttop', 'path='. $full_url .'&product_id=' . $product_id));
		}
	}
