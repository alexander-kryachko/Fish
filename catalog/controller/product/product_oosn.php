...
class ControllerProductProductOosn extends Controller {
	...
	public function index() {
		...
		if  ($formset == 1 && $pid){
			...
			$mail->setSubject($this->config->get('hb_oosn_admin_email_subject_'.$admin_language), ENT_QUOTES, 'UTF-8');
...
public function sendNotificationtoCustomers($oosn_id){
	...
	foreach ($emaillists as $emaillist){
		...
		if (!empty($pimage)){
			...
			$mail_subject = str_replace("{customer_name}",$customer_name,$mail_subject);
			...
			if (preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $customer_email)) {					
					...
					//$mail->setFrom($from);	
					$mail->setFrom($mail->username);	// set smtp login as sender (fix 15.03.17)
					
					$mail->setSender($store_name);//Sender
					$mail->setSubject(html_entity_decode($mail_subject, ENT_QUOTES, 'UTF-8'));					
					$mail->setHtml($message);
					$error = $mail->send();
					if (!$error) echo 'Notification Email sent to '.$customer_email.' <br>';
						else echo 'Skipped address '.$customer_email.' <br>';
			}

			//if (($this->config->get('hb_oosn_sms_enable') == 'y') and (strlen($customer_phone) == $this->config->get('hb_oosn_mobile_validation'))){
			if (($this->config->get('hb_oosn_sms_enable') == 'y') and (strlen($customer_phone) >= 10)){
				$sms =  $this->config->get('hb_oosn_customer_sms_body_'.$customer_language_id);
				//$pname = (strlen($pname) > 20) ? mb_substr($pname,0,20,'utf-8').'...' : $pname;
				//$pmodel = (strlen($pmodel) > 10) ? mb_substr($pmodel,0,10,'utf-8').'...' : $pmodel;
				//$selected_option = (strlen($selected_option) > 15) ? mb_substr($selected_option,0,14,'utf-8').'...' : $selected_option;
				//$all_selected_option = (strlen($all_selected_option) > 15) ? mb_substr($all_selected_option,0,14,'utf-8').'...' : $all_selected_option;
			    ...
			    $sms = str_replace('/\s+/', ' ',$sms);
				//$sms = iconv('utf-8', 'windows-1251', $sms);
				//$sms = iconv('windows-1251', 'utf-8', $sms);
				//$sms = iconv("UTF-8", "UTF-8//IGNORE", $sms);
				$sms = rawurlencode($sms);
				
				$url = 'http://alphasms.com.ua/api/http.php?version=http&key=750421d2b628cd0019fb3bf7b4d8bca5ea5182d5&command=send&from=FisherWay&to={to}&message={msg}';//html_entity_decode($this->config->get('config_hb_sms_http_api'));
...
public function checkstock() {
	...
	public function emaildemandedproducts(){
      ...
      <tr>
		...
		$this->hbemail('HTML',$this->config->get('config_email'), $this->config->get('config_email'), 'Demanded Out-of-Stock Products in your store', $html);
