...
class ControllerFeedPromua extends Controller {
	...
	private function generateYml() {
		if ($this->config->get('prom_ua_status')) {
			...
			$this->load->model('export/prom_ua');
			...
			//$this->setShop('name', $this->config->get('prom_ua_shopname'));
			$this->setShop('company', $this->config->get('config_owner'));
			//$this->setShop('company', $this->config->get('prom_ua_company'));
			...
			$this->setShop('version', prom_ua2_VERSION);
		
			// Валюты
			// TODO: Добавить возможность настраивать проценты в админке.
			$offers_currency = $this->config->get('prom_ua_currency');
			...
			$datamodel = $this->config->get('prom_ua_datamodel');
			
			// Категории
			$categories = $this->model_export_prom_ua->getCategory();
			...
			$in_stock_id = $this->config->get('prom_ua_in_stock'); // id статуса товара "В наличии"
			$out_of_stock_id = $this->config->get('prom_ua_out_of_stock'); // id статуса товара "Нет на складе"
			$vendor_required = ($datamodel == 'vendor_model'); // true - только товары у которых задан производитель, необходимо для 'vendor.model' 

			$pickup = ($this->config->get('prom_ua_pickup') ? 'true' : false);
			
			if ($this->config->get('prom_ua_delivery_cost') != '') {
				$local_delivery_cost = intval($this->config->get('prom_ua_delivery_cost'));
			...
			$store = ($this->config->get('prom_ua_store') ? 'true' : false);
			$unavailable = $this->config->get('prom_ua_unavailable');

			$allowed_categories = $this->config->get('prom_ua_categories');
			$allowed_manufacturers = $this->config->get('prom_ua_manufacturers');
			$blacklist_type = $this->config->get('prom_ua_blacklist_type');
			$blacklist = $this->config->get('prom_ua_blacklist');
			$products = $this->model_export_prom_ua->getProduct($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_id, $vendor_required, $allowed_manufacturers);

			$numpictures = $this->config->get('prom_ua_numpictures');
			if ($numpictures > 1) {
				//++++ Дополнительные изображения товара ++++
				$product_images = $this->model_export_prom_ua->getProductImages($numpictures - 1);
				//---- Дополнительные изображения товара ----
			}
			$all_attributes = $this->model_export_prom_ua->getAttributes($this->config->get('prom_ua_attributes'));
			$this->optioned_name = $this->config->get('prom_ua_optioned_name');
			
			$prom_ua_categ_mapping = unserialize($this->config->get('prom_ua_categ_mapping'));
			
			$this->color_options = explode(',', $this->config->get('prom_ua_color_options'));
			$this->size_options = explode(',', $this->config->get('prom_ua_size_options'));
			$this->size_units = $this->config->get('prom_ua_size_units') ? unserialize($this->config->get('prom_ua_size_units')) : array();
			...
			foreach ($products as $product) {
                ...
                } else if(!empty($product['master_product_id']) || $product['master_product_id'] != -1){
                    $data['group_id'] = $product['master_product_id'];
                } else {
                    $data['group_id'] = false;
                }
...
//				$data['cbid'] = 15;
				...
				$querycats = $this->model_catalog_product->getCategories($product['product_id']);
				$categories = array();
				...
				$utmsource = $this->config->get('prom_ua_utmsource');
				$data['url'] = $this->url->link('product/product', 'path=' . $this->getPath($product['category_id']) . '&product_id=' . $product['product_id'] . '&utm_source=' . $utmsource . '&utm_medium=price&utm_term=' . $product['name'] . '&utm_content=' . $product['manufacturer'] . '&utm_campaign=' . $product_category_name);
				$data['price'] = $product['price'];
				if ($data['price'] == 0) continue;
				$data['currencyId'] = $offers_currency;
				$data['categoryId'] = $product['category_id'];
				if (isset($prom_ua_categ_mapping[$product['category_id']]) && $prom_ua_categ_mapping[$product['category_id']]) {
					$data['market_category'] = $prom_ua_categ_mapping[$product['category_id']];
				}
				$data['delivery'] = 'true';
				...
				$product_description = strip_tags($product[prom_ua2_DESCRIPTION_FIELD]);
				if (prom_ua2_SHORTER_DESCRIPTION > 0) {
					$product_description = mb_substr($product_description, 0, prom_ua2_SHORTER_DESCRIPTION, 'UTF-8');
				}
				$data['description'] = $product_description;
				$sales_notes = $this->config->get('prom_ua_sales_notes');
				if ($sales_notes) {
...
//				$data['barcode'] = $product['sku'];
			...
			//	++++ Атрибуты товара ++++
				...
				$attributes = $this->model_export_prom_ua->getProductAttributes($product['product_id']);
				if (count($attributes) > 0) {
					foreach ($attributes as $attr) {
						if ($attr['attribute_id'] == $this->config->get('prom_ua_adult')) {
							$data['adult'] = 'true';
						}
						elseif ($attr['attribute_id'] == $this->config->get('prom_ua_manufacturer_warranty')) {
							$data['manufacturer_warranty'] = 'true';
						}
						elseif ($attr['attribute_id'] == $this->config->get('prom_ua_country_of_origin')) {
	...
	protected function setOptionedOffer($data, $product, $shop_currency, $offers_currency, $decimal_place) {
		if (!$this->color_options)
			return false;
		$offers_array = array();
		$coptions = $this->model_export_prom_ua->getProductOptions($this->color_options, $product['product_id']);
		if (!count($coptions))
			return false;
		...
		if ($this->size_options)
			$soptions = $this->model_export_prom_ua->getProductOptions($this->size_options, $product['product_id']);
		$idx = 0;
	...
	private function prepareImage($image) {
		...
		if (is_file(DIR_IMAGE . $image)) {
			...
			if ($width_orig < 600 || $height_orig < 600 || !prom_ua2_ORIGINAL_IMAGES)
				return $this->model_tool_image->resize($image, 600, 600);
			else
				return HTTP_IMAGE . str_replace(' ', '%20', $image);
		}
		return false;
	}
	...
	private function setOffer($data) {
		...
		$attributes = array('id', 'group_id ', 'type', 'available', 'bid', 'cbid', 'param', 'group_id');
