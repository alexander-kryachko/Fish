...
class ControllerFeedElama extends Controller {
	...
	private function generateYml() {
		if ($this->config->get('elama_status')) {
			...
			//$this->setShop('name', $this->config->get('elama_shopname'));
			$this->setShop('company', $this->config->get('config_owner'));
			//$this->setShop('company', $this->config->get('elama_company'));
			...
			$this->setShop('version', elama2_VERSION);
		
			// Валюты
			// TODO: Добавить возможность настраивать проценты в админке.
			$offers_currency = $this->config->get('elama_currency');
			...
			$datamodel = $this->config->get('elama_datamodel');
			
			// Категории
			$categories = $this->model_export_elama->getCategory();
			...
			$in_stock_id = $this->config->get('elama_in_stock'); // id статуса товара "В наличии"
			$out_of_stock_id = $this->config->get('elama_out_of_stock'); // id статуса товара "Нет на складе"
			$vendor_required = ($datamodel == 'vendor_model'); // true - только товары у которых задан производитель, необходимо для 'vendor.model' 

			$pickup = ($this->config->get('elama_pickup') ? 'true' : false);
			
			if ($this->config->get('elama_delivery_cost') != '') {
				$local_delivery_cost = intval($this->config->get('elama_delivery_cost'));
			...
			$store = ($this->config->get('elama_store') ? 'true' : false);
			$unavailable = $this->config->get('elama_unavailable');

			$allowed_categories = $this->config->get('elama_categories');
			$allowed_manufacturers = $this->config->get('elama_manufacturers');
			$blacklist_type = $this->config->get('elama_blacklist_type');
			$blacklist = $this->config->get('elama_blacklist');
			$products = $this->model_export_elama->getProduct($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_id, $vendor_required, $allowed_manufacturers);

			$numpictures = $this->config->get('elama_numpictures');
			if ($numpictures > 1) {
				//++++ Дополнительные изображения товара ++++
				$product_images = $this->model_export_elama->getProductImages($numpictures - 1);
				//---- Дополнительные изображения товара ----
			}
			$all_attributes = $this->model_export_elama->getAttributes($this->config->get('elama_attributes'));
			$this->optioned_name = $this->config->get('elama_optioned_name');
			
			$elama_categ_mapping = unserialize($this->config->get('elama_categ_mapping'));
			
			$this->color_options = explode(',', $this->config->get('elama_color_options'));
			$this->size_options = explode(',', $this->config->get('elama_size_options'));
			$this->size_units = $this->config->get('elama_size_units') ? unserialize($this->config->get('elama_size_units')) : array();
...
//				$data['cbid'] = 15;
				...
				$utmsource = $this->config->get('elama_utmsource');
				...
				if (isset($elama_categ_mapping[$product['category_id']]) && $elama_categ_mapping[$product['category_id']]) {
					$data['market_category'] = $elama_categ_mapping[$product['category_id']];
				}
				$data['delivery'] = 'true';
				...
				$product_description = strip_tags($product[elama2_DESCRIPTION_FIELD]);
				if (elama2_SHORTER_DESCRIPTION > 0) {
					$product_description = mb_substr($product_description, 0, elama2_SHORTER_DESCRIPTION, 'UTF-8');
				}
				$data['description'] = $product_description;
				$sales_notes = $this->config->get('elama_sales_notes');
				if ($sales_notes) {
...
//				$data['barcode'] = $product['sku'];
				...
				*/
				$data['param'] = array();
				$attributes = $this->model_export_elama->getProductAttributes($product['product_id']);
				if (count($attributes) > 0) {
					foreach ($attributes as $attr) {
						if ($attr['attribute_id'] == $this->config->get('elama_adult')) {
							$data['adult'] = 'true';
						}
						elseif ($attr['attribute_id'] == $this->config->get('elama_manufacturer_warranty')) {
							$data['manufacturer_warranty'] = 'true';
						}
						elseif ($attr['attribute_id'] == $this->config->get('elama_country_of_origin')) {
	...
	protected function setOptionedOffer($data, $product, $shop_currency, $offers_currency, $decimal_place) {
		if (!$this->color_options)
			return false;
		$offers_array = array();
		$coptions = $this->model_export_elama->getProductOptions($this->color_options, $product['product_id']);
		if (!count($coptions))
			return false;
		...
		if ($this->size_options)
			$soptions = $this->model_export_elama->getProductOptions($this->size_options, $product['product_id']);
		$idx = 0;
	...
	private function prepareImage($image) {
		...
		if (is_file(DIR_IMAGE . $image)) {
			...
			if ($width_orig < 600 || $height_orig < 600 || !elama2_ORIGINAL_IMAGES)
