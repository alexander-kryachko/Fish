...
class ControllerFeedNadaviNet extends Controller {
	...
	private function generateYml() {
		if ($this->config->get('nadavi_net_status')) {
			...
			//$this->setShop('name', $this->config->get('nadavi_net_shopname'));
			$this->setShop('company', $this->config->get('config_owner'));
			//$this->setShop('company', $this->config->get('nadavi_net_company'));
			...
			$this->setShop('version', YANDEX_YML2_VERSION);

			// Валюты
			// TODO: Добавить возможность настраивать проценты в админке.
			$offers_currency = $this->config->get('nadavi_net_currency');
			...
			$datamodel = $this->config->get('nadavi_net_datamodel');
			...
			$in_stock_id = $this->config->get('nadavi_net_in_stock'); // id статуса товара "В наличии"
			$out_of_stock_id = $this->config->get('nadavi_net_out_of_stock'); // id статуса товара "Нет на складе"
			$vendor_required = ($datamodel == 'vendor_model'); // true - только товары у которых задан производитель, необходимо для 'vendor.model' 

			$pickup = ($this->config->get('nadavi_net_pickup') ? 'true' : false);
			
			if ($this->config->get('nadavi_net_delivery_cost') != '') {
				$local_delivery_cost = intval($this->config->get('nadavi_net_delivery_cost'));
			...
			$store = ($this->config->get('nadavi_net_store') ? 'true' : false);
			$unavailable = $this->config->get('nadavi_net_unavailable');

			$allowed_categories = $this->config->get('nadavi_net_categories');
			$allowed_manufacturers = $this->config->get('nadavi_net_manufacturers');
			$blacklist_type = $this->config->get('nadavi_net_blacklist_type');
			$blacklist = $this->config->get('nadavi_net_blacklist');
			...
			$numpictures = $this->config->get('nadavi_net_numpictures');
			if ($numpictures > 1) {
				//++++ Дополнительные изображения товара ++++
				$product_images = $this->model_export_prom_ua->getProductImages($numpictures - 1);
				//---- Дополнительные изображения товара ----
			}
			$all_attributes = $this->model_export_prom_ua->getAttributes($this->config->get('nadavi_net_attributes'));
			$this->optioned_name = $this->config->get('nadavi_net_optioned_name');
			
			$yandex_yml_categ_mapping = unserialize($this->config->get('nadavi_net_categ_mapping'));
			
			$this->color_options = explode(',', $this->config->get('nadavi_net_color_options'));
			$this->size_options = explode(',', $this->config->get('nadavi_net_size_options'));
			$this->size_units = $this->config->get('nadavi_net_size_units') ? unserialize($this->config->get('nadavi_net_size_units')) : array();
...
//				$data['cbid'] = 15;
				...
				$utmsource = $this->config->get('nadavi_net_utmsource');
				...
				$product_description = strip_tags($product[YANDEX_YML2_DESCRIPTION_FIELD]);
				if (YANDEX_YML2_SHORTER_DESCRIPTION > 0) {
					$product_description = mb_substr($product_description, 0, YANDEX_YML2_SHORTER_DESCRIPTION, 'UTF-8');
				}
				$data['description'] = $product_description;
				$sales_notes = $this->config->get('nadavi_net_sales_notes');
				if ($sales_notes) {
...
//				$data['barcode'] = $product['sku'];
				...
				*/
				...
				if (count($attributes) > 0) {
					foreach ($attributes as $attr) {
						if ($attr['attribute_id'] == $this->config->get('nadavi_net_adult')) {
							$data['adult'] = 'true';
						}
						elseif ($attr['attribute_id'] == $this->config->get('nadavi_net_manufacturer_warranty')) {
							$data['manufacturer_warranty'] = 'true';
						}
						elseif ($attr['attribute_id'] == $this->config->get('nadavi_net_country_of_origin')) {
	...
	private function prepareImage($image) {
		...
		if (is_file(DIR_IMAGE . $image)) {
			...
			if ($width_orig < 600 || $height_orig < 600 || !YANDEX_YML2_ORIGINAL_IMAGES)
