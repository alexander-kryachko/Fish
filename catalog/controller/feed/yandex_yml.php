...
class ControllerFeedYandexYml extends Controller {
	...
	private function generateYml() {
		if ($this->config->get('yandex_yml_status')) {
			...
			$allowed_manufacturers = $this->config->get('yandex_yml_manufacturers');
			$blacklist_type = $this->config->get('yandex_yml_blacklist_type');
...
//				$data['cbid'] = 15;
				...
				$utmsource = $this->config->get('yandex_yml_utmsource');
...
//				$data['barcode'] = $product['sku'];
				...
				*/
				...
				//++++ Описание товара ++++
				if ($this->config->get('yandex_yml_attr_vs_description')) {
					$data['description'] = implode($attr_text,"\n");
				}
				else {
					$product_description = strip_tags($product[YANDEX_YML_DESCRIPTION_FIELD]);
					if (YANDEX_YML_SHORTER_DESCRIPTION > 0) {
						$product_description = mb_substr($product_description, 0, YANDEX_YML_SHORTER_DESCRIPTION, 'UTF-8');
					}
