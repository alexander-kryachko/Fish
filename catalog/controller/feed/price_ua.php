...
class ControllerFeedPriceUa extends Controller {
	...
	private function generateYml() {
		if ($this->config->get('price_ua_status')) {
			...
			//$this->setShop('company', $this->config->get('price_ua_company'));
			...
			$offers_currency = $this->config->get('price_ua_currency');
			...
			$datamodel = $this->config->get('price_ua_datamodel');
			...
			$pickup = ($this->config->get('price_ua_pickup') ? 'true' : false);
			
			if ($this->config->get('price_ua_delivery_cost') != '') {
				$local_delivery_cost = intval($this->config->get('price_ua_delivery_cost'));
			...
			$store = ($this->config->get('price_ua_store') ? 'true' : false);
			$unavailable = $this->config->get('price_ua_unavailable');

			$allowed_categories = $this->config->get('price_ua_categories');
			$allowed_manufacturers = $this->config->get('price_ua_manufacturers');
			$blacklist_type = $this->config->get('price_ua_blacklist_type');
			$blacklist = $this->config->get('price_ua_blacklist');
			...
			$numpictures = $this->config->get('price_ua_numpictures');
			if ($numpictures > 1) {
				//++++ Дополнительные изображения товара ++++
				$product_images = $this->model_export_yandex_yml->getProductImages($numpictures - 1);
				//---- Дополнительные изображения товара ----
			}
			$all_attributes = $this->model_export_yandex_yml->getAttributes($this->config->get('price_ua_attributes'));
			$this->optioned_name = $this->config->get('price_ua_optioned_name');
			
			$yandex_yml_categ_mapping = unserialize($this->config->get('price_ua_categ_mapping'));
			
			$this->color_options = explode(',', $this->config->get('price_ua_color_options'));
			$this->size_options = explode(',', $this->config->get('price_ua_size_options'));
			$this->size_units = $this->config->get('price_ua_size_units') ? unserialize($this->config->get('price_ua_size_units')) : array();
...
//				$data['cbid'] = 15;
				...
				$utmsource = $this->config->get('price_ua_utmsource');
				...
				$sales_notes = $this->config->get('price_ua_sales_notes');
				if ($sales_notes) {
				...
				*/
				...
				if (count($attributes) > 0) {
					foreach ($attributes as $attr) {
						if ($attr['attribute_id'] == $this->config->get('price_ua_adult')) {
							$data['adult'] = 'true';
						}
						elseif ($attr['attribute_id'] == $this->config->get('price_ua_manufacturer_warranty')) {
							$data['manufacturer_warranty'] = 'true';
						}
						elseif ($attr['attribute_id'] == $this->config->get('price_ua_country_of_origin')) {
