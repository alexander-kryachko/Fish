...
class ControllerFeedTrafmag extends Controller {
	...
	private function generateYml() {
		if ($this->config->get('trafmag_status')) {
			...
			//$this->setShop('name', $this->config->get('trafmag_shopname'));
			$this->setShop('company', $this->config->get('config_owner'));
			//$this->setShop('company', $this->config->get('trafmag_company'));
			...
			$offers_currency = $this->config->get('trafmag_currency');
			...
			$datamodel = $this->config->get('trafmag_datamodel');
			...
			$in_stock_id = $this->config->get('trafmag_in_stock'); // id статуса товара "В наличии"
			$out_of_stock_id = $this->config->get('trafmag_out_of_stock'); // id статуса товара "Нет на складе"
			$vendor_required = ($datamodel == 'vendor_model'); // true - только товары у которых задан производитель, необходимо для 'vendor.model' 

			$pickup = ($this->config->get('trafmag_pickup') ? 'true' : false);
			
			if ($this->config->get('trafmag_delivery_cost') != '') {
				$local_delivery_cost = intval($this->config->get('trafmag_delivery_cost'));
			...
			$store = ($this->config->get('trafmag_store') ? 'true' : false);
			$unavailable = $this->config->get('trafmag_unavailable');

			$allowed_categories = $this->config->get('trafmag_categories');
			$allowed_manufacturers = $this->config->get('trafmag_manufacturers');
			$blacklist_type = $this->config->get('trafmag_blacklist_type');
			$blacklist = $this->config->get('trafmag_blacklist');
			...
			$numpictures = $this->config->get('trafmag_numpictures');
			if ($numpictures > 1) {
				//++++ Дополнительные изображения товара ++++
				$product_images = $this->model_export_yandex_yml->getProductImages($numpictures - 1);
				//---- Дополнительные изображения товара ----
			}
			$all_attributes = $this->model_export_yandex_yml->getAttributes($this->config->get('trafmag_attributes'));
			$this->optioned_name = $this->config->get('trafmag_optioned_name');
			
			$yandex_yml_categ_mapping = unserialize($this->config->get('trafmag_categ_mapping'));
			
			$this->color_options = explode(',', $this->config->get('trafmag_color_options'));
			$this->size_options = explode(',', $this->config->get('trafmag_size_options'));
			$this->size_units = $this->config->get('trafmag_size_units') ? unserialize($this->config->get('trafmag_size_units')) : array();
...
//				$data['cbid'] = 15;
				...
				$utmsource = $this->config->get('trafmag_utmsource');
				...
				$sales_notes = $this->config->get('trafmag_sales_notes');
				if ($sales_notes) {
				...
				*/
				...
				if (count($attributes) > 0) {
					foreach ($attributes as $attr) {
						if ($attr['attribute_id'] == $this->config->get('trafmag_adult')) {
							$data['adult'] = 'true';
						}
						elseif ($attr['attribute_id'] == $this->config->get('trafmag_manufacturer_warranty')) {
							$data['manufacturer_warranty'] = 'true';
						}
						elseif ($attr['attribute_id'] == $this->config->get('trafmag_country_of_origin')) {
