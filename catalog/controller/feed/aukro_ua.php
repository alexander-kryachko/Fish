...
class ControllerFeedAukroUa extends Controller {
	private $shop = array();
	...
	private function generateYml() {
		if ($this->config->get('aukro_ua_status')) {
			...
			$this->load->model('export/aukro_ua');
			...
			//$this->setShop('name', $this->config->get('aukro_ua_shopname'));
			$this->setShop('company', $this->config->get('config_owner'));
			//$this->setShop('company', $this->config->get('aukro_ua_company'));
			...
			$offers_currency = $this->config->get('aukro_ua_currency');
			...
			$datamodel = $this->config->get('aukro_ua_datamodel');
			
			// Категории
			$categories = $this->model_export_aukro_ua->getCategory();
			...
			$in_stock_id = $this->config->get('aukro_ua_in_stock'); // id статуса товара "В наличии"
			$out_of_stock_id = $this->config->get('aukro_ua_out_of_stock'); // id статуса товара "Нет на складе"
			$vendor_required = ($datamodel == 'vendor_model'); // true - только товары у которых задан производитель, необходимо для 'vendor.model' 

			$pickup = ($this->config->get('aukro_ua_pickup') ? 'true' : false);
			
			if ($this->config->get('aukro_ua_delivery_cost') != '') {
				$local_delivery_cost = intval($this->config->get('aukro_ua_delivery_cost'));
			...
			$store = ($this->config->get('aukro_ua_store') ? 'true' : false);
			$unavailable = $this->config->get('aukro_ua_unavailable');

			$allowed_categories = $this->config->get('aukro_ua_categories');
			$allowed_manufacturers = $this->config->get('aukro_ua_manufacturers');
			$blacklist_type = $this->config->get('aukro_ua_blacklist_type');
			$blacklist = $this->config->get('aukro_ua_blacklist');
			$products = $this->model_export_aukro_ua->getProduct($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_id, $vendor_required, $allowed_manufacturers);

			$numpictures = $this->config->get('aukro_ua_numpictures');
			if ($numpictures > 1) {
				//++++ Дополнительные изображения товара ++++
				$product_images = $this->model_export_aukro_ua->getProductImages($numpictures - 1);
				//---- Дополнительные изображения товара ----
			}
			$all_attributes = $this->model_export_aukro_ua->getAttributes($this->config->get('aukro_ua_attributes'));
			$this->optioned_name = $this->config->get('aukro_ua_optioned_name');
			
			$yandex_yml_categ_mapping = unserialize($this->config->get('aukro_ua_categ_mapping'));
			
			$this->color_options = explode(',', $this->config->get('aukro_ua_color_options'));
			$this->size_options = explode(',', $this->config->get('aukro_ua_size_options'));
			$this->size_units = $this->config->get('aukro_ua_size_units') ? unserialize($this->config->get('aukro_ua_size_units')) : array();
...
//				$data['cbid'] = 15;
				...
				$utmsource = $this->config->get('aukro_ua_utmsource');
				...
				$sales_notes = $this->config->get('aukro_ua_sales_notes');
				if ($sales_notes) {
...
//				$data['barcode'] = $product['sku'];
				...
				*/
				$data['param'] = array();
				$attributes = $this->model_export_aukro_ua->getProductAttributes($product['product_id']);
				if (count($attributes) > 0) {
					foreach ($attributes as $attr) {
						if ($attr['attribute_id'] == $this->config->get('aukro_ua_adult')) {
							$data['adult'] = 'true';
						}
						elseif ($attr['attribute_id'] == $this->config->get('aukro_ua_manufacturer_warranty')) {
							$data['manufacturer_warranty'] = 'true';
						}
						elseif ($attr['attribute_id'] == $this->config->get('aukro_ua_country_of_origin')) {
	...
	protected function setOptionedOffer($data, $product, $shop_currency, $offers_currency, $decimal_place) {
		if (!$this->color_options)
			return false;
		$offers_array = array();
		$coptions = $this->model_export_aukro_ua->getProductOptions($this->color_options, $product['product_id']);
		if (!count($coptions))
			return false;
		...
		if ($this->size_options)
			$soptions = $this->model_export_aukro_ua->getProductOptions($this->size_options, $product['product_id']);
		$idx = 0;
