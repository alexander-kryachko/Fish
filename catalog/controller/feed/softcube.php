...
class ControllerFeedSoftcube extends Controller {
	...
	private function generateYml() {
		if ($this->config->get('softcube_status')) {
			...
			//$this->setShop('name', $this->config->get('softcube_shopname'));
			$this->setShop('company', $this->config->get('config_owner'));
			//$this->setShop('company', $this->config->get('softcube_company'));
			...
			$offers_currency = $this->config->get('softcube_currency');
			...
			$datamodel = $this->config->get('softcube_datamodel');
			...
			$in_stock_id = $this->config->get('softcube_in_stock'); // id статуса товара "В наличии"
			$out_of_stock_id = $this->config->get('softcube_out_of_stock'); // id статуса товара "Нет на складе"
			$vendor_required = ($datamodel == 'vendor_model'); // true - только товары у которых задан производитель, необходимо для 'vendor.model' 

			$pickup = ($this->config->get('softcube_pickup') ? 'true' : false);
			
			if ($this->config->get('softcube_delivery_cost') != '') {
				$local_delivery_cost = intval($this->config->get('softcube_delivery_cost'));
			...
			$store = ($this->config->get('softcube_store') ? 'true' : false);
			$unavailable = $this->config->get('softcube_unavailable');

			$allowed_categories = $this->config->get('softcube_categories');
			$allowed_manufacturers = $this->config->get('softcube_manufacturers');
			$blacklist_type = $this->config->get('softcube_blacklist_type');
			$blacklist = $this->config->get('softcube_blacklist');
			$products = $this->model_export_yandex_yml->getProduct($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_id, $vendor_required, $allowed_manufacturers);

			$numpictures = $this->config->get('softcube_numpictures');
			if ($numpictures > 1) {
				//++++ Дополнительные изображения товара ++++
				$product_images = $this->model_export_yandex_yml->getProductImages($numpictures - 1);
				//---- Дополнительные изображения товара ----
			}
			$all_attributes = $this->model_export_yandex_yml->getAttributes($this->config->get('softcube_attributes'));
			$this->optioned_name = $this->config->get('softcube_optioned_name');
			
			$yandex_yml_categ_mapping = unserialize($this->config->get('softcube_categ_mapping'));
			
			$this->color_options = explode(',', $this->config->get('softcube_color_options'));
			$this->size_options = explode(',', $this->config->get('softcube_size_options'));
			$this->size_units = $this->config->get('softcube_size_units') ? unserialize($this->config->get('softcube_size_units')) : array();
...
//				$data['cbid'] = 15;
				...
				$utmsource = $this->config->get('softcube_utmsource');
				$data['url'] = $this->url->link('product/product', 'path=' . $this->getPath($product['category_id']) . '&product_id=' . $product['product_id'] . '&utm_source=' . $utmsource . '&utm_medium=retargeting&utm_term=' . $product['name'] . '&utm_content=' . $product['manufacturer'] . '&utm_campaign=' . $product_category_name);
				...
				$sales_notes = $this->config->get('softcube_sales_notes');
				if ($sales_notes) {
				...
				*/
				...
				if (count($attributes) > 0) {
					foreach ($attributes as $attr) {
						if ($attr['attribute_id'] == $this->config->get('softcube_adult')) {
							$data['adult'] = 'true';
						}
						elseif ($attr['attribute_id'] == $this->config->get('softcube_manufacturer_warranty')) {
							$data['manufacturer_warranty'] = 'true';
						}
						elseif ($attr['attribute_id'] == $this->config->get('softcube_country_of_origin')) {
