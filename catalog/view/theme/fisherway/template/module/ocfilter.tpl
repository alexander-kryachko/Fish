<?php 
global $ocAliases;
if (!function_exists('oclink')){
	function oclink($url, $options, $params = false){
		global $ocAliases;
		$i = strpos($url, '?');
		if ($i === false) $i = strlen($url);

		$prefix = substr($url, 0, $i);
		$postfix = substr($url, $i);
		$str = false;
		if (!empty($params)){
			...
			$orderedParams = array();
			foreach($params as $p){
				$t = explode(':', $p);// preg_replace("/:(.*)/", '', $p);
				$n = array_search($t[0], $optionKeys);
				$orderedParams[$n] = $t;
			}
			ksort($orderedParams);
			foreach($orderedParams as $t){
				if (empty($t[1])) continue;

				$optionValues = array();

            $orderedVals = array();
			if( array_key_exists($t[0],$options)  ) {
                $optionKeys = $options[$t[0]]['values'];

                foreach ($optionKeys as $v) $optionValues[] = $v['value_id'];

                $vals = explode(',', $t[1]);
                //$orderedVals = array();
                foreach ($vals as $v) {
                    $n = array_search($v, $optionValues);
                    $orderedVals[$n] = $v;
                }
                ksort($orderedVals);
            }
				$addVals = array();
				foreach($orderedVals as $v) if (isset($ocAliases[$v])) $addVals[] = $ocAliases[$v];
				if (!empty($addVals)) $str .= '/'.implode(',', $addVals);
			}
			$host = 'http://'.$_SERVER['HTTP_HOST'].'/';
			$result = $host.strtr(substr($prefix, strlen($host)).$str.$postfix, array('/' => '_'));
			return $result;
		}
...
<div class="box ocfilter ocfilter-<?php echo $position; ?>" id="ocfilter-<?php echo $module; ?>">
  ...
  <div class="box-content">
    <div class="relative-wrapper">
			<div class="button-float" id="button-float-<?php echo $module; ?>"><a href="#" id="button-submit-<?php echo $module; ?>">Низкие цены, быстрая доставка</a></div>
		</div>
    <form action="">
      ...
      <?php } ?>
			...
			<? if (in_array($option['type'], array('slide', 'slide_dual')) || !empty($option['values'])){ ?>
            ...
            <?php } elseif ($option['type'] == 'slide' || $option['type'] == 'slide_dual') { # Slide type start ?>
	          ...
	          <?php foreach ($option['values'] as $key => $value) { ?>
						...
						<?php if ($show_values_limit && $show_values_limit == $key) { 
							...
							<div class="hidden-values"><?php
						} 

						if ($option['color']) { 
							?><div class="color" style="background-color: #<?php echo $value['color']; ?>;"></div><?php 
						} 

						if ($option['image']) { 
							?><div class="image" style="background-image: url(<?php echo $value['image']; ?>);"></div><?php 
						} 
							$baseLink = $link;//'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
							$i = 0;
							if (!empty($_GET)) foreach($_GET as $k => $v){
								if (in_array($k, array('_route_', 'filter_ocfilter'))) continue;
								$i++;
							}
							if ($value['selected']) {			
								?><label id="v-<?php echo $value['id']; ?>" class="selected"><input type="<?php echo $option['type']; ?>" name="option[<?php echo $option['option_id']; ?>]" value="<?php echo $value['params']; ?>" checked="checked" class="ocf-target" /><a href="<?=oclink($baseLink, $options, !empty($value['params']) ? $value['params'] : false) /*$baseLink.(!empty($value['params']) ? ($i ? '&amp;' : '?').'filter_ocfilter='.$value['params'] : '')*/?>" class="ocFilterLink" onClick="ga('send', 'event', {'eventCategory': 'Filters', 'eventAction': '<?php echo $option['name']; ?>', 'eventLabel': '<?php echo $value['name']; ?>'})"><?php echo $value['name']; ?></a><?php if ($show_counter) { ?><small></small><?php } ?></label><?php 
								
							} elseif ($value['count']) { 
								?><label id="v-<?php echo $value['id']; ?>"><input type="<?php echo $option['type']; ?>" name="option[<?php echo $option['option_id']; ?>]" value="<?php echo $value['params']; ?>" class="ocf-target" /><a href="<?=oclink($baseLink, $options, !empty($value['params']) ? $value['params'] : false) /*$baseLink.(!empty($value['params']) ? ($i ? '&amp;' : '?').'filter_ocfilter='.$value['params'] : '')*/?>" class="ocFilterLink" onClick="ga('send', 'event', 'Filters', '<?php echo $option['name']; ?>', '<?php echo $value['name']; ?>')"><?php echo $value['name']; ?></a><?php if ($show_counter) { ?><small><?php echo $value['count']; ?></small><?php } ?></label><?php 

							} else { 
								?><label id="v-<?php echo $value['id']; ?>" class="disabled"><input type="<?php echo $option['type']; ?>" name="option[<?php echo $option['option_id']; ?>]" value="<?php echo $value['params']; ?>" disabled="disabled" class="ocf-target" /><?php echo $value['name']; ?><?php if ($show_counter) { ?><small>0</small><?php } ?></label><?php 
							} ?>
