  ...
  <div class="product-info" itemscope itemtype="http://schema.org/Product">
     <div class="left left-product">
		...
		if ($images) {
			...
			echo '<div class="scrollable"><div class="items">';
			$n = 0;
      ...
      */
      ?>
				...
				if ($is_master) {
					...
					if(sizeof($pds) > 0) {
						...
						foreach ($pds as $p){
							...
							if ($oldmin === false || $p_all['price'] < $oldmin) $oldmin = $p_all['price'];
						} 
						if ($min != 999999) {
        ...
        <?php } ?>
          ...
          <?php echo 'Купи ' .$discount['quantity']. ' шт ' .$discount['price']. '' ?> <span class="economy"><?php // echo ($price * $discount['quantity']) ?></span><br />
          <?php } ?>
        </div>
            <?php if ($quantity != 0) { ?>
	  ...
	  <?php } ?>
      <div class="depDate" ><?php echo $stickers; ?>
		    <?
		  if ($manufacturer == 'Favorite' && !empty($stickers)){
			echo '<a class="size-table my_popup_open" id="size-table-link"><span>Официальный продавец ТМ Favorite</span></a>';
...
<!-- \nostock update -->				  
      ...
      <?php } ?>
		  ...
		  <p><b>Купи сейчас и получи:</b></p>
		  <div class="statuses" style="margin-top: 10px;margin-left: 23px;margin-bottom: 10px;"><?php echo $statuses; ?></div>
							...
							<div class="buy">
							   ...
							   $p_all = $this->model_catalog_product->getProduct($p['product_id']); 
								if ($p['quantity'] != 0){
								   //BOF: old price
								   if ($p_all['special']){
										echo '<del class="old-price">&nbsp;'.round($p_all['price']).' <span>грн</span>&nbsp;</del><br />';
								   }
								   //EOF: old price
								...
								<?
								if (!empty($p_all['sku'])){
									echo '<div class="model-sku">Код: '.ltrim($p_all['sku'],'0').'</div>';
								}
								?>
...
<tr>
  ...
  <div class="breadcrumb breadcrumbBottom">
    <?php
