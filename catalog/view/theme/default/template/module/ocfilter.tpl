            ...
            <?php } elseif ($option['type'] == 'slide' || $option['type'] == 'slide_dual') { # Slide type start ?>
	          ...
	          <?php foreach ($option['values'] as $key => $value) { ?>
	            ...
	            <?php if ($option['image']) { ?>
							...
							if ($value['selected']) {
							?>
		          <label id="v-<?php echo $value['id']; ?>" class="selected"><input type="<?php echo $option['type']; ?>" name="option[<?php echo $option['option_id']; ?>]" value="<?php echo $value['params']; ?>" checked="checked" class="ocf-target" /><a href="<?=oclink($baseLink, $options, !empty($value['params']) ? $value['params'] : false) /*$baseLink.(!empty($value['params']) ? ($i ? '&amp;' : '?').'filter_ocfilter='.$value['params'] : '')*/?>" class="ocFilterLink" onClick="ga('send', 'event', {'eventCategory': 'Filters', 'eventAction': '<?php echo $option['name']; ?>', 'eventLabel': '<?php echo $value['name']; ?>'})"><?php echo $value['name']; ?></a><?php if ($show_counter) { ?><small></small><?php } ?></label>
							<?php } elseif ($value['count']) { 
							?>
		          <label id="v-<?php echo $value['id']; ?>"><input type="<?php echo $option['type']; ?>" name="option[<?php echo $option['option_id']; ?>]" value="<?php echo $value['params']; ?>" class="ocf-target" /><a href="<?=oclink($baseLink, $options, !empty($value['params']) ? $value['params'] : false) /*$baseLink.(!empty($value['params']) ? ($i ? '&amp;' : '?').'filter_ocfilter='.$value['params'] : '')*/?>" class="ocFilterLink" onClick="ga('send', 'event', 'Filters', '<?php echo $option['name']; ?>', '<?php echo $value['name']; ?>')"><?php echo $value['name']; ?></a><?php if ($show_counter) { ?><small><?php echo $value['count']; ?></small><?php } ?></label>
							<?php } else { ?>
