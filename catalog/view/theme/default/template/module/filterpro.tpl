...
<form id="filterpro" action="<?php echo $filter_redirect?>" method="POST">

<input type="hidden" data-name="fp_redirect" value="<?php echo $filter_redirect?>">

<div class="option_box" <?php if(!$instock_checked) echo 'style="display:none"'; ?>>
		...
		<table>
			<tr>
				<td><input type="checkbox" class="filtered" name="instock" id="instock"/><label for="instock"></label>
					<label for="instock"><span class="label-name"><?php echo $text_instock?></span></label></td>
			</tr>
		</table>
	...
	<?php }?>
<input type="hidden" name="page" id="filterpro_page" value="<?php echo $page?>">
...
<input type="hidden" name="limit" id="filterpro_limit" value="<?php echo $limit?>">
...
<div class="option_name price-filter"><?php echo $text_price_range?></div>
<div class="price_slider collapsible">
<span class="table-price">
<table>
	<tr>
		<?php if($symbol_left){ echo "<td><label>". $symbol_left  . "</label></td>";}?>
		<td><input type="text" id="min_price" value="<?php echo $min_price?>" name="min_price" class="price_limit"></td>
		<td><label> - </label></td>
		<td><input type="text" id="max_price" value="<?php echo $max_price?>" name="max_price" class="price_limit"></td>
	...
	<?php if($filter_groups) { ?>
		<?php if($display_filter == 'select') { ?>
			<?php foreach($filter_groups as $filter_group) { ?>
				<div class="option_box filter_box">
					<div class="option_name <?php if(!$expanded_filter) { echo "hided"; } ?>">
						<?php echo $filter_group['name']; ?>
					</div>
					<div class="collapsible" <?php if(!$expanded_filter) { echo 'style="display:none"'; } ?>>
						<select class="filtered" name="filter[<?php echo $filter_group['filter_group_id']; ?>][]">
							<option value=""><?php echo $text_all ?></option>
							<?php foreach($filter_group['filter'] as $filter) { ?>
								<option class="filter_value" id="filter_value_<?php echo $filter['filter_id'] ?>"
										value="<?php echo $filter['filter_id']; ?>"><?php echo $filter['name']; ?></option>
							<?php } ?>
						</select>
					</div>
				</div>
			<?php } ?>
		<?php } elseif($display_filter == 'checkbox') { ?>
		<ul class="box-filter option_box">
		  <?php foreach ($filter_groups as $filter_group) { ?>
		  <li><span id="filter-group<?php echo $filter_group['filter_group_id']; ?>" class="option_name <?php if(!$expanded_filter){echo "hided";}?>"><?php echo $filter_group['name']; ?></span>
			<ul class="collapsible" <?php if(!$expanded_filter){echo 'style="display:none"';}?>>
			  <?php foreach ($filter_group['filter'] as $filter) { ?>
			  <li>
				<input type="checkbox" class="filtered" name="filter[<?php echo $filter_group['filter_group_id']; ?>][]" value="<?php echo $filter['filter_id']; ?>" id="filter<?php echo $filter['filter_id']; ?>" />
				  <label for="filter<?php echo $filter['filter_id']; ?>"></label><label for="filter<?php echo $filter['filter_id']; ?>"><span class="label-name"><?php echo $filter['name']; ?></span><span class="label-value"></span></label>
			  </li>
			  <?php } ?>
			</ul>
		  </li>
		...
		<table id="filter_categories">
			...
			<tr>
				<td>
					...
					<label for="cat_<?php echo $category['category_id']; ?>"></label>
					<label for="cat_<?php echo $category['category_id']; ?>"><span class="label-name"><?php echo $category['name']; ?></span><span class="label-value"><?php echo $category['total']+1; ?></span></label>
				</td>
			</tr>
			<?php } ?>
		</table>
		...
		<table id="filter_tags">
			...
			<tr>
				<td>
					...
					<label for="tag_<?php echo $tag['tag']; ?>"></label>
					<label for="tag_<?php echo $tag['tag']; ?>"><span class="label-name"><?php echo $tag['name']; ?></span></label>
				</td>
			</tr>
			<?php } ?>
		</table>
		...
		<div>
			<select name="manufacturer[]" class="filtered">
				...
				<option id="manufacturer_<?php echo $manufacturer['manufacturer_id']?>" class="manufacturer_value"
						value="<?php echo $manufacturer['manufacturer_id']?>"><?php echo $manufacturer['name']?> <?php echo $manufacturer['total']?></option>
				<?php } ?>
			</select>
		</div>
		...
		<table>
			...
			<tr>
				<td>
					...
					<label for="manufacturer_<?php echo $manufacturer['manufacturer_id']?>"></label>
					<label for="manufacturer_<?php echo $manufacturer['manufacturer_id']?>"><span class="label-name"><?php echo $manufacturer['name']?></span><span class="label-value"><?php echo $manufacturer['total']?></span></label>
				</td>
			</tr>
			<?php } ?>
		</table>
			...
			<div class="collapsible" <?php if(!$attribute_value['expanded']){echo 'style="display:none"';}?>>
				...
				<div>
					<select class="filtered" name="attribute_value[<?php echo $attribute_value_id?>][]">
						...
						<option class="a_name"
								at_v_i="<?php echo $attribute_value_id . '_' . $value['a'] ?>"
								at_v_t="<?php echo $attribute_value_id . '_' . htmlspecialchars(preg_replace('/\s+|\n|\r|\s+$/m', '_', $value['a'])) ?>"
								data-value="<?php echo $value['a'] ?>"
								value="<?php echo $value['a'] ?>"><?php echo $value['a'] ?></option>
						<?php }?>
					</select>
				</div>
					...
					<tr>
						<td>
							<input class="filtered a_name"
								   id="attribute_value_<?php echo $attribute_value_id . "_" . $i; ?>"
								   type="checkbox" name="attribute_value[<?php echo $attribute_value_id?>][]"
								   at_v_i="<?php echo $attribute_value_id . '_' . $value['a']; ?>"
								   value="<?php echo $value['a'] ?>">
							<label for="attribute_value_<?php echo $attribute_value_id . "_" . $i; ?>"></label>
							<label for="attribute_value_<?php echo $attribute_value_id . "_" . $i; ?>"
								   at_v_t="<?php echo $attribute_value_id . '_' . htmlspecialchars(preg_replace('/\s+|\n|\r|\s+$/m', '_', $value['a'])); ?>"
								   data-value="<?php echo strip_tags(html_entity_decode($value['a'], ENT_QUOTES)); ?>"
								   value="<?php echo $value['a'] ?>"><span class="label-name"><?php echo strip_tags(html_entity_decode($value['a'], ENT_QUOTES))?></span><span class="label-value"><?php echo $value['t'] ?></span></label>
						</td>
					</tr>
					<?php } ?>
				</table>
					...
					<tr>
						...
						<td>
							<label for="attribute_value_<?php echo $attribute_value_id . "_" . $i; ?>"
								   at_v_t="<?php echo $attribute_value_id . '_' . htmlspecialchars(preg_replace('/\s+|\n|\r|\s+$/m', '_', $value['a'])) ?>"
								   data-value="<?php echo $value['a'] ?>"
								   value="<?php echo $value['a'] ?>"><span class="label-name"><?php echo $value['a']?></span><span class="label-value"><?php echo $value['t'] ?></span></label>
						</td>
					</tr>
					<?php } ?>
				</table>
	...
	<div class="option_box">
		...
		<table>
			...
			<tr>
				<td>
					...
					<label for="option_value_<?php echo $option_value['option_value_id']?>"></label>
					<label for="option_value_<?php echo $option_value['option_value_id']?>"><span class="label-name"><?php echo $option_value['name']?></span><span class="label-value"><?php echo $option_value['total']?></span></label>
				</td>
			</tr>
			<?php } ?>
		</table>
		...
		<table class="collapsible" <?php if(!$option['expanded']){echo 'style="display:none"';}?>>
			...
			<tr>
				...
				<td>
					<label for="option_value_<?php echo $option_value['option_value_id']?>"><span class="label-name"><?php echo $option_value['name']?></span></label>
				</td>
			</tr>
			<?php } ?>
		</table>
		...
		<div class="collapsible" <?php if(!$option['expanded']) { echo 'style="display:none"'; }?>>
			<?php foreach($option['option_values'] as $option_value) { ?>
					...
					<input style="display: none;" class="filtered option_value" id="option_value_<?php echo $option_value['option_value_id']?>"
						   type="checkbox" name="option_value[<?php echo $option['option_id']?>][]"
						   value="<?php echo $option_value['option_value_id']?>">
					<img src="<?php echo $option_value['thumb'];?>" title="<?php echo $option_value['name'];?>" alt="<?php echo $option_value['name'];?>"/>
		...
		<?php } ?>
	<?php } ?>
<?php if ($filer_special) {?>
	<div class="option_box">
		<div class="option_name"><?php echo $text_filer_special?></div>
		<div class="collapsible">
			<input type="checkbox" class="filtered" name="filer_special" id="filer_special"><label for="filer_special"></label><label for="filer_special"><?php echo $text_show_filer_special?></label>
		</div>
	</div>
<?php }?>
<?php if ($filer_news) {?>
<div class="option_box">
	...
	<div class="collapsible">
		<input type="checkbox" class="filtered" name="filer_news" id="filer_news"><label for="filer_news"></label><label for="filer_news"><?php echo $text_show_filer_news ?></label>
	</div>
</div>
	<?php } ?>
</form>
<?php if($noajax) { ?>
<div class="cart" style="margin: 10px 0px 10px 15px;">
<input type="button" class="button" onclick="$('#filterpro_page').val(1);noAjaxFilter();" value="Применить">
</div>
<?php } ?>
</div>
	<a class="clear_filter"><?php echo $clear_filter?></a>
</div>
