	      ...
	      if (target.is('select')) {
		    ...
		    if ($this.options.php.showButton) {
		      ...
		      if ($this.options.php.params) {
				var gi = url.indexOf('?') >= 0 ? true : false;
				url += (gi ? '&' : '?') + $this.options.php.index + '=' + $this.options.php.params;
			}
	  ...
	  update: function() {
		  ...
		  $.get('index.php?route=module/ocfilter/callback', data, function(json){
		    ...
		    if (json.total === 0) {
		      ...
		      $this.options.element.submitButton.attr('href', $this.options.php.link + ($this.options.php.params ? (($this.options.php.link.indexOf('?') >= 0 ? '&' : '?') + $this.options.php.index + '=' + $this.options.php.params) : '')).removeClass('disabled').text(json.text_total);
