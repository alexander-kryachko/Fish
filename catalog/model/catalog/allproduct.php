...
class ModelCatalogAllproduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			$query->row['price'] = $query->row['discount'] ? ($query->row['special'] > 0 && $query->row['special'] < $query->row['price'] ? $query->row['special'] : $query->row['price'])*$query->row['discount']/100 : $query->row['price'];
