<?php
class ModelExportPromUa extends Model {
	...
	public function getProduct($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_id, $vendor_required = true, $allowed_manufacturers = '') {
		...
		$query = $this->db->query("SELECT
			p.*, pd.name, pd.description, pd.meta_description, m.name AS manufacturer, p2c.category_id, IFNULL(ps.price, p.price) AS price, wcd.unit AS weight_unit, pm.master_product_id AS master_product_id
