...
define('PAGE_CACHE_VERSION', '1.6.0');

define('PAGE_CACHING', TRUE);		// TRUE/FALSE - IF FALSE, CACHING IS DISABLED
...
define('ADMIN_BAR_TOP', FALSE);		// TRUE/FALSE - TRUE = BAR AT TOP, FALSE = BAR AT BOTTOM
define('AUTO_EXPIRE_TIME', 3);	// NUMBER OF HOURS AFTER WHICH A CACHE FILE AUTO EXPIRES, 0 = INFINITE
define('OCPC_SESH_SWITCH', TRUE);	// SET TO TRUE IF YOU WANT TO HAVE THE TIMER BAR ON MULTISTORE
define('OCPC_USER_TIMES', TRUE);	// TRUE/FALSE - SET TO FALSE TO HIDE SOURCE CODE TIMES FROM STANDARD USERS
...
define('OCPC_SUB_FOLDERS', TRUE);	// TRUE/FALSE - SET TO FALSE TO DISABLE SUBFOLDERS IN THE CACHEFILES DIRECTORY
define('OCPC_CACHE_HTTPS', FALSE);	// TRUE/FALSE - SET TO TRUE IF YOU WANT TO CACHE HTTPS URL's FALSE BY DEFAULT
define('OCPC_DEVICE_DETECT', FALSE);// TRUE/FALSE - SET TO TRUE TO DETECT MOBILE AND TABLETS AS WELL AS DESKTOP USERS
define('OCPC_CACHE_MODE', 0777);	// SET TO SAME PERMISSIONS AS YOUR /system/cache/ folder permissions (needs to be writeable)
...
if(PAGE_CACHING) {
	...
	// Get Device Type if necessary
	$device = '';
	if(OCPC_DEVICE_DETECT) {
		include('Mobile_Detect.php');
		$device_type = new Mobile_Detect();
		
		if($device_type->isTablet()) {
			$device = '.tab';
		} elseif($device_type->isMobile()) {
			$device = '.mob';
		}
	}
	...
	header('X-JG-Page-Cache: ' . PAGE_CACHE_VERSION);
	
	// Check request is a GET call
	if(!empty($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'GET' && (OCPC_CACHE_HTTPS || (empty($_SERVER['HTTPS']) || $_SERVER['HTTPS'] == 'off'))) {
		...
		if(empty($_SESSION['customer_id']) && empty($_SESSION['affiliate_id']) && empty($_GET['affiliate']) && empty($_SESSION['cart']) && empty($_SESSION['vouchers'])) {
			...
			$requestKey = sha1($key) . $device;
			
			$dirPath = DIR_PAGE_CACHE;
			if(OCPC_SUB_FOLDERS) {
				$dirPath .=  substr($requestKey, 0, 2) . '/';
				if(!is_dir($dirPath)) {
					$old = umask(0);
					mkdir($dirPath, OCPC_CACHE_MODE);
					umask($old);
				}
			}
			...
			// Cache filename
			$pageCacheFile = $dirPath . 'cache.' . $requestKey;
...
function addBar($content, $requestKey) {
	...
	if(!empty($_SESSION['user_id']) && !empty($_SESSION['token'])) {
		...
		$sub = OCPC_SUB_FOLDERS ? 1 : 0;
		
		$bar = <<<BARCONTENT
	<body\$1><div id="pageCacheBar" style="">
		<p style="margin: 0;"><input type="button" onclick="clearcache()" value="Delete this pages cache file" /> Without Caching: {without}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;With Caching: {with}</p>
	</div>
...
function clearcache() {
	$.get(
		...
		function(data) {
			if(parseInt(data) == 0) {
				alert('Error - Cache file no longer exists. This may have been deleted already');
			} else {
				alert('Cache file successfully deleted');
			}
		}
	);
}
