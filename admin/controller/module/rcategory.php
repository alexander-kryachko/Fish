...
class ControllerModuleRcategory extends Controller {
	...
	public function index() {
		...
		$this->data['is_installed'] = $this->model_module_rcategory->showTable('category_related');

		$this->data['create'] = $this->url->link('module/rcategory/create', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function create() {
		$this->load->model('module/rcategory');
		if ($this->model_module_rcategory->showTable('category_related')) {
			$this->session->data['error'] = 'Таблицы уже существуют';
		} else {
			$this->model_module_rcategory->createTable();
			$this->session->data['success'] = 'Создание таблицы прошло успешно';
		}
		$this->redirect($this->url->link('module/rcategory', 'token=' . $this->session->data['token'], 'SSL'));
	}
