<?php
class ControllerModuleCachemanager extends Controller {
	private $error = array();
	...
	public function index() {   
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/cachemanager', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/cachemanager', 'token=' . $this->session->data['token'], 'SSL');
		
		$this->data['clearcache'] = (HTTPS_SERVER . 'index.php?route=module/cachemanager/clearcache&token=' . $this->session->data['token']);
		$this->data['clearsystemcache'] = (HTTPS_SERVER . 'index.php?route=module/cachemanager/clearsystemcache&token=' . $this->session->data['token']);
		$this->data['clearvqmodcache'] = (HTTPS_SERVER . 'index.php?route=module/cachemanager/clearvqmodcache&token=' . $this->session->data['token']);
		...
		if  ($this->config->get('cache')) {
			  $this->data['cache'] = $this->config->get('cache');
				...
				$this->data['cache']['bestsellermodule']['status'] = 1;
				$this->data['cache']['bestsellermodule']['lifetime'] = 3600;		
				$this->data['cache']['latestmodule']['status'] = 1;
				$this->data['cache']['latestmodule']['lifetime'] = 3600*12;	
				$this->data['cache']['specialmodule']['status'] = 1;
				$this->data['cache']['specialmodule']['lifetime'] = 3600*12;
				$this->data['cache']['featuredmodule']['status'] = 1;
				$this->data['cache']['featuredmodule']['lifetime'] = 3600*24;
				
				$this->data['cache']['productcategory']['status'] = 1;
				$this->data['cache']['productcategory']['lifetime'] = 3600;
				
				$this->data['cache']['productmanufacturer']['status'] = 1;
				$this->data['cache']['productmanufacturer']['lifetime'] = 3600;
		}	
		
		
		$this->data['clear_menu'] = (HTTPS_SERVER . 'index.php?route=module/cachemanager/clear&pattern=menu.&token=' . $this->session->data['token']);
		$this->data['clear_categorymodule'] = (HTTPS_SERVER . 'index.php?route=module/cachemanager/clear&pattern=module.categorymodule.&token=' . $this->session->data['token']);
		$this->data['clear_featuredmodule'] = (HTTPS_SERVER . 'index.php?route=module/cachemanager/clear&pattern=module.featuredmodule.&token=' . $this->session->data['token']);
		$this->data['clear_bestsellermodule'] = (HTTPS_SERVER . 'index.php?route=module/cachemanager/clear&pattern=module.bestsellermodule.&token=' . $this->session->data['token']);
		$this->data['clear_latestmodule'] = (HTTPS_SERVER . 'index.php?route=module/cachemanager/clear&pattern=module.latestmodule.&token=' . $this->session->data['token']);
		$this->data['clear_specialmodule'] = (HTTPS_SERVER . 'index.php?route=module/cachemanager/clear&pattern=module.specialmodule.&token=' . $this->session->data['token']);
		$this->data['clear_productcategory'] = (HTTPS_SERVER . 'index.php?route=module/cachemanager/clear&pattern=product.category.&token=' . $this->session->data['token']);
		$this->data['clear_productmanufacturer'] = (HTTPS_SERVER . 'index.php?route=module/cachemanager/clear&pattern=product.manufacturer.&token=' . $this->session->data['token']);
		...
		$this->template = 'module/cachemanager.tpl';
		$this->children = array(
			'common/header',	
			'common/footer'	
		);
	...
	public function clear() {
		...
		$this->redirect(HTTPS_SERVER . 'index.php?route=module/cachemanager&token=' . $this->session->data['token']);		
	}
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/cachemanager')) {
