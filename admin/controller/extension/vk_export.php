 ...
 * 
 * Данная копия модуля зарегистрирована на 
 * 
 */
      ...
      //  $link = $http_catalog . 'index.php?route=product/product&product_id=' . $p['product_id'];
        ...
        */
        ...
        if ($special_price) {
            ...
            $special_price = $this->currency->format($special_price);
            $price = sprintf($special_text, $special_price, $price);
        }
        
        // теги товара как хеш-теги
        $hashtags = array();
        foreach (explode(',', $product_descriptions[$lang_id]['tag']) as $k => $v) {
            $hashtags[] = '#' . str_replace(' ', '_', trim($v));
        }
