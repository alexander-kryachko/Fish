...
class ControllerSaleCoupon extends Controller {
	...
	private function getDiscountPlan($post) {
		if ($post['type'] == 'C' && is_array($post['discount_b']) && count($post['discount_b']) > 1) {
			$plan = array();
			foreach($post['discount_b'] as $i=>$b) {
				if ($i > 0) $plan[floatval($b)] = floatval($post['discount_v'][$i]);
			}
			ksort($plan);
			return $plan;
		}
	...
	private function generateCode($prefix, $digits) {
  	...
  	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && !empty($this->request->post['action'])){
			switch($this->request->post['action']){
				case 'addOrder':
					echo $this->model_sale_coupon->addOrder($this->request->get['coupon_id'], $this->request->post['order']);
					exit();
				case 'deleteOrder':
					echo $this->model_sale_coupon->deleteOrder($this->request->get['coupon_id'], $this->request->post['order']);
					exit();
			}
		}
