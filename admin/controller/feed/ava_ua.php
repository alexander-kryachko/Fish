...
class ControllerFeedAvaUa extends Controller {
	...
	public function index() {
		$this->load->language('feed/ava_ua');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate($this->request->post))) {
			...
			foreach($this->request->post as $key=>$val) {
				$key = str_replace('yandex_yml_', 'ava_ua_', $key);
				$to_save[$key] = $val;
			}
			$this->model_setting_setting->editSetting('ava_ua', $to_save);
		...
		$this->data['cron_path'] = 'php '.realpath(DIR_CATALOG.'../export/ava_ua.php');
		$this->data['entry_export_url'] = $this->language->get('entry_export_url');
		$this->data['export_url'] = HTTP_CATALOG.'export/ava_ua.xml';
		...
		$this->data['breadcrumbs'][] = array(
			'href'      => $this->url->link('feed/ava_ua', 'token=' . $this->session->data['token'], 'SSL'),
			'text'      => $this->language->get('heading_title'),
			'separator' => ' :: '
		);

		$this->data['action'] = $this->url->link('feed/ava_ua', 'token=' . $this->session->data['token'], 'SSL');
		...
		*/
		...
		if (isset($this->request->post['yandex_yml_delivery_cost'])) {
			...
			$this->data['yandex_yml_delivery_cost'] = $this->config->get('ava_ua_delivery_cost');
		}

		if (isset($this->request->post['yandex_yml_currency'])) {
			$this->data['yandex_yml_currency'] = $this->request->post['yandex_yml_currency'];
		} else {
			$this->data['yandex_yml_currency'] = $this->config->get('ava_ua_currency');
		}
		...
		if (isset($this->request->post['yandex_yml_blacklist'])) {
			...
			$blacklist = explode(',', $this->config->get('ava_ua_blacklist'));
		}
		...
		} elseif ($this->config->get('ava_ua_unavailable')) {
			$this->data['yandex_yml_unavailable'] = $this->config->get('ava_ua_unavailable');
		} else {
		...
		} elseif ($this->config->get('ava_ua_in_stock')) {
			$this->data['yandex_yml_in_stock'] = $this->config->get('ava_ua_in_stock');
		} else {
		...
		if (isset($this->request->post['yandex_yml_out_of_stock'])) {
			...
			$this->data['yandex_yml_out_of_stock'] = $this->config->get('ava_ua_out_of_stock');
		} else {
		...
		if (isset($this->request->post['yandex_yml_pickup'])) {
			...
			$this->data['yandex_yml_pickup'] = $this->config->get('ava_ua_pickup');
		}
		...
		if (isset($this->request->post['yandex_yml_numpictures'])) {
			...
			$this->data['yandex_yml_numpictures'] = $this->config->get('ava_ua_numpictures');
		}
		...
		} elseif ($this->config->get('ava_ua_attributes') != '') {
			$this->data['yandex_yml_attributes'] = explode(',', $this->config->get('ava_ua_attributes'));
		} else {
		...
		if (isset($this->request->post['yandex_yml_adult'])) {
			...
			$this->data['yandex_yml_adult'] = $this->config->get('ava_ua_adult');
		}
		if (isset($this->request->post['yandex_yml_manufacturer_warranty'])) {
			...
			$this->data['yandex_yml_manufacturer_warranty'] = $this->config->get('ava_ua_manufacturer_warranty');
		}
		if (isset($this->request->post['yandex_yml_country_of_origin'])) {
			...
			$this->data['yandex_yml_country_of_origin'] = $this->config->get('ava_ua_country_of_origin');
		}
		...
		} elseif ($this->config->get('ava_ua_color_options') != '') {
			$this->data['yandex_yml_color_options'] = explode(',', $this->config->get('ava_ua_color_options'));
		} else {
			$this->data['yandex_yml_color_options'] = array();
		}
		if (isset($this->request->post['yandex_yml_size_options'])) {
			$this->data['yandex_yml_size_options'] = $this->request->post['yandex_yml_size_options'];
		} elseif ($this->config->get('ava_ua_size_options') != '') {
			$this->data['yandex_yml_size_options'] = explode(',', $this->config->get('ava_ua_size_options'));
		} else {
			$this->data['yandex_yml_size_options'] = array();
		}
		if (isset($this->request->post['yandex_yml_size_units'])) {
			$this->data['yandex_yml_size_units'] = $this->request->post['yandex_yml_size_units'];
		} elseif ($this->config->get('ava_ua_size_units') != '') {
			$this->data['yandex_yml_size_units'] = unserialize($this->config->get('ava_ua_size_units'));
		} else {
		...
		if (isset($this->request->post['yandex_yml_optioned_name'])) {
			...
			$this->data['yandex_yml_optioned_name'] = $this->config->get('ava_ua_optioned_name');
		}
		...
		} elseif ($this->config->get('ava_ua_categories') != '') {
			$this->data['yandex_yml_categories'] = explode(',', $this->config->get('ava_ua_categories'));
		} else {
			$this->data['yandex_yml_categories'] = array();
		}
		if (isset($this->request->post['yandex_yml_categ_mapping'])) {
			$this->data['yandex_yml_categ_mapping'] = $this->request->post['yandex_yml_categ_mapping'];
		} elseif ($this->config->get('ava_ua_categ_mapping') != '') {
			$this->data['yandex_yml_categ_mapping'] = unserialize($this->config->get('ava_ua_categ_mapping'));
		} else {
			$this->data['yandex_yml_categ_mapping'] = array();
		}
		if (isset($this->request->post['yandex_yml_manufacturers'])) {
			$this->data['yandex_yml_manufacturers'] = $this->request->post['yandex_yml_manufacturers'];
		} elseif ($this->config->get('ava_ua_manufacturers') != '') {
			$this->data['yandex_yml_manufacturers'] = explode(',', $this->config->get('ava_ua_manufacturers'));
		} else {
	...
	private function validate($data) {
		if (!$this->user->hasPermission('modify', 'feed/ava_ua')) {
