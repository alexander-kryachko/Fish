...
class ModelToolExchange1c extends Model {
	...
	public function queryOrders($params){
		...
		if ($params['exchange_status'] != 0) {
			...
			// test
			if ($_SERVER['REMOTE_ADDR'] == '77.122.30.199') $query = $this->db->query("SELECT order_id FROM `" . DB_PREFIX . "order` WHERE `order_id` = 9311");
		} else {
...
//				
		...
		// test
		if ($_SERVER['REMOTE_ADDR'] == '77.122.30.199') print_r($document);
