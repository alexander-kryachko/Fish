...
class ModelToolExchange1c extends Model {
	...
	public function queryOrders($params){
		...
		if ($params['exchange_status'] != 0) {
			...
			// test
			//if ($_SERVER['REMOTE_ADDR'] == '77.122.7.164') $query = $this->db->query("SELECT order_id FROM `" . DB_PREFIX . "order` WHERE `order_id` = 646");
		} else {
...
//				
				...
				foreach ($products as $product) {
					...
					$n = 0;
					if (!empty($order_options)) foreach($order_options as $option){
						$product_characteristics['ХарактеристикаТовара'.($n ? $n : '')] = array('Значение' => $option['value']);
						$n++;
					}
					...
					$document['Документ' . $document_counter]['Товары']['Товар' . $product_counter] = array(
						...
						,'ХарактеристикиТовара' => $product_characteristics
				...
				foreach ($totals as $total){
					...
					if ($total['code']=='set'){
						...
						if (!isset($document['Документ' . $document_counter]['ЗначенияРеквизитов'])) $document['Документ' . $document_counter]['ЗначенияРеквизитов'] = array();
						$document['Документ' . $document_counter]['ЗначенияРеквизитов']['ЗначениеРеквизита6'] = array(
							'Наименование' => 'Скидка от суммы заказа',
							'Значение' => $total['title']
						);
					}
				}
				...
				if ($discount_sum > 0){
					...
					$document['Документ' . $document_counter]['Скидки']['Скидка'] = array(
						'Ид' => ''
						,'Наименование' => 'Общая скидка'
						,'Сумма' => $discount_sum
						, 'Комментарий' => 'Общая скидка'
						, 'УчтеноВСумме' => '1'
					);
				}
...
//				
		...
		// test
		//if ($_SERVER['REMOTE_ADDR'] == '77.122.7.164') print_r($document);

		//return $xml->asXML();
		return $this->fixEncoding($xml->asXML());
	}
	private function fixEncoding($str){
		...
		$m = self::str_split($str);
		$result = '';
		for($i=0, $len = count($m[0]); $i<$len; $i++) $result .= in_array($m[0][$i], $allowed) || is_numeric($m[0][$i]) ? $m[0][$i] : ' ';
		$result = preg_replace('/ +/', ' ', $result);
		$result = trim($result, ' ');
		return $result;
	}
	private static function str_split($str){
