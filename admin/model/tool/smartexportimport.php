...
class ModelToolSmartExportImport extends Model {
	...
	function uploadProducts( &$reader, &$database ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			//FB::Log($availableProductIds);
			//验证是否存在重复的产品
			//$this->language->load('tool/smartexportimport');
			if(isset($availablePorductIds)){
				if (in_array((int)$productId,$availablePorductIds)) {
					/*echo '<script type="text/javascript">
							alert("';
					echo sprintf($this->language->get('error_exist_product'), $productId);
					echo '");history.go(-1);
						</script>';
						exit;*/
						...
						$sql .= "DELETE FROM `".DB_PREFIX."product_description` WHERE language_id=$languageId AND `product_id` = '$productId' ;\n";
						...
						$sql .= "DELETE FROM `".DB_PREFIX."url_alias` WHERE `query` LIKE 'product_id=$productId' ;\n";
						...
						$sql .= "DELETE FROM `".DB_PREFIX."product_attribute` WHERE language_id=$languageId AND `product_id` = '$productId' ;\n";
	...
	function upload( $filename ) {
		...
		try {
			...
			// Memory Optimization of processing begin sunboy	
		...
		//Optimization of processing end sunboy
