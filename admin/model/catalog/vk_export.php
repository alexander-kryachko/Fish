...
class ModelCatalogVkExport extends Model {
    ...
    public function getProductsAutoexport() {
        $sql = "SELECT p.*, pd.*, ptc.category_id
			...
			and pm.master_product_id in (0, -1)
    ...
    public function getProductsAutoWallPost() {
        ...
        if ($this->config->get('vk_export_wall_only_specials')) {
            $sql .= ' OR (SELECT COUNT(*) from ' . DB_PREFIX . 'product_special WHERE product_id = p.product_id AND date_start <> 0000-00-00 AND p.`quantity` > 0)';
        }  
        if ($this->config->get('vk_export_only_instock')) {
            $sql .= ' AND p.`quantity` > 0';
        }
	...
	public function getPath_old($category_id) {
		...
		if ($query->row['parent_id']) {
			return $this->getPath_old($query->row['parent_id'], $this->config->get('config_language_id')) . $this->language->get('text_separator') . $query->row['name'];
		} else {
			return $query->row['name'];
		}
	}
    
    public function license_install($vk_export_license_key, $vk_export_license_login, $vk_export_license_email) {
        // vk_export_license_key
        $res = $this->db->query("SELECT * FROM " . DB_PREFIX . "setting
            WHERE `group` = 'vk_export' AND `key` = 'vk_export_license_key'
        ");
        if (!$res->num_rows) {
            $this->db->query("INSERT " . DB_PREFIX . "setting SET value = '" . $this->db->escape($vk_export_license_key) . "',
                 `group` = 'vk_export', `key` = 'vk_export_license_key'
            ");
        }
        // vk_export_license_login
        $res = $this->db->query("SELECT * FROM " . DB_PREFIX . "setting
            WHERE `group` = 'vk_export' AND `key` = 'vk_export_license_login'
        ");
        if (!$res->num_rows) {
            $this->db->query("INSERT " . DB_PREFIX . "setting SET value = '" . $this->db->escape($vk_export_license_login) . "',
                 `group` = 'vk_export', `key` = 'vk_export_license_login'
            ");
        }
        // vk_export_license_email
        $res = $this->db->query("SELECT * FROM " . DB_PREFIX . "setting
            WHERE `group` = 'vk_export' AND `key` = 'vk_export_license_email'
        ");
        if (!$res->num_rows) {
            $this->db->query("INSERT " . DB_PREFIX . "setting SET value = '" . $this->db->escape($vk_export_license_email) . "',
                 `group` = 'vk_export', `key` = 'vk_export_license_email'
            ");
        }
    }
