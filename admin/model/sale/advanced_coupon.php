...
class ModelSaleAdvancedCoupon extends Model {
	...
	public function serializeAdvancedCouponOptions($data) {
		...
		$option = array();
		$option['advanced_coupon'] = $data['advanced_coupon'];
		if (isset($data['category_buy'])) {
			$option['category_buy'] = $data['category_buy'];	
		} else {
			$option['category_buy'] = array();
		}
		
		if (isset($data['manufacturer_buy'])) {
			$option['manufacturer_buy'] = $data['manufacturer_buy'];	
		} else {
			$option['manufacturer_buy'] = array();
		}
		...
		if (isset($data['manufacturer'])) {
			$option['manufacturer'] = $data['manufacturer'];	
		} else {
			$option['manufacturer'] = array();
		}
		...
		$option['product_type'] = $data['product_type'];
		$option['product_buy_type'] = $data['product_buy_type'];
		...
		//$option['discount_rules_type'] = $data['discount_rules_type'];
		//$option['discount_rules_value'] = $data['discount_rules_value'];
		//$option['discount_rules'] = $data['discount_rules'];
	...
	public function addAdvancedCoupon($data) {
		...
		$language_data = $this->db->query("SELECT language_id FROM `" . DB_PREFIX . "language` WHERE code='" . $language_code . "'");
		
		$language_id = $language_data->row['language_id'];
		
		$this->db->query("INSERT INTO " . DB_PREFIX . "advanced_coupon SET name = '" . $this->db->escape($data['advanced_coupon'][$language_id]['name']) . "', code = '" . $this->db->escape($data['code']) . "', options = '" . $this->db->escape($options) . "', date_start = '" . $this->db->escape($data['date_start']) . "', date_end = '" . $this->db->escape($data['date_end']) . "', status = '" . (int)$data['status'] . "', date_added = NOW()");
	...
	public function editAdvancedCoupon($advanced_coupon_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "advanced_coupon SET name = '" . $this->db->escape($data['advanced_coupon'][$language_id]['name']) . "', code = '" . $this->db->escape($data['code']) . "', options = '" . $this->db->escape($options) . "', date_start = '" . $this->db->escape($data['date_start']) . "', date_end = '" . $this->db->escape($data['date_end']) . "', status = '" . (int)$data['status'] . "' WHERE advanced_coupon_id = '" . (int)$advanced_coupon_id . "'");
	...
	public function getAdvanceCouponData($advance_coupon_id)
	{
		$sql = $this->db->query("SELECT * FROM `" . DB_PREFIX . "advanced_coupon` WHERE advanced_coupon_id='" . (int)$advance_coupon_id. "'");
		
		$option = unserialize($sql->row['options']);
		
		//print "<pre>"; print_r($option); exit;
		
		return $option['advanced_coupon'];
