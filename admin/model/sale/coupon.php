...
class ModelSaleCoupon extends Model {
	...
	public function addOrder($coupon_id, $order_id){
      	$oq = $this->db->query("SELECT customer_id, total FROM " . DB_PREFIX . "order WHERE order_id='" . (int) $order_id ."'");
      	if (!$oq->num_rows) return 'Order not found';

		$cq = $this->db->query("SELECT coupon_history_id FROM " . DB_PREFIX . "coupon_history WHERE order_id='" . (int) $order_id ."' AND coupon_id = '".(int) $coupon_id."' LIMIT 1");
		if ($cq->num_rows) return 'Order already added';

		$this->db->query("INSERT INTO " . DB_PREFIX . "coupon_history(coupon_id, order_id, customer_id, amount, date_added) VALUES (
			".(int) $coupon_id.",
			".(int) $order_id.",
			".(int) $oq->row['customer_id'].",
			".(float) $oq->row['total'].",
			NOW()
		)");
	}
	
	public function deleteOrder($coupon_id, $order_id){
		$cq = $this->db->query("DELETE FROM " . DB_PREFIX . "coupon_history WHERE order_id='" . (int) $order_id ."' AND coupon_id = '".(int) $coupon_id."' LIMIT 1");
	}
