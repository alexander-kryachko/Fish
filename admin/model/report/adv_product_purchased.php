<?php
class ModelReportAdvProductPurchased extends Model {
	public function getProductPurchased($data = array()) {
	...
	public function getProductSaleChart($data = array()) {
		...
		SUM(op.total) AS gtotal
		
		FROM `" . DB_PREFIX . "order_product` op, `" . DB_PREFIX . "order` o 
		
		WHERE op.order_id = o.order_id" . $date . $sdate . $osi . $order_id . $store . $cur . $tax . $tclass . $geo_zone . $cgrp . $stat . $cust . $email . $tel . $ip . $pcomp . $paddr . $pcity . $pzone . $ppsc . $pcntr . $pmeth . $scomp . $saddr . $scity . $szone . $spsc . $scntr . $smeth . $cat . $manu . $sku . $prod . $mod . $opt . $atr . $stat . $loc . $affn . $affe . $cpn . $cpc . $gvc;	
	...
	public function getOrderPaymentMethods($data = array()) {
		$query = $this->db->query("SELECT DISTINCT o.payment_method AS payment_name, HEX(o.payment_method) AS payment_title FROM `" . DB_PREFIX . "order` o WHERE o.order_status_id > 0 AND o.payment_code IS NOT NULL AND o.payment_code !='' ORDER BY LCASE(o.payment_method) ASC");
		
		return $query->rows;	
	}
	
	public function getOrderShippingMethods($data = array()) {
		$query = $this->db->query("SELECT DISTINCT o.shipping_method AS shipping_name, HEX(o.shipping_method) AS shipping_title FROM `" . DB_PREFIX . "order` o WHERE o.order_status_id > 0 AND o.shipping_code IS NOT NULL AND o.shipping_code !='' ORDER BY LCASE(o.shipping_method) ASC");
