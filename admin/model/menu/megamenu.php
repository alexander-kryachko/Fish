...
class ModelMenuMegamenu extends Model { 
	 ...
	 */
	public function getTree( $id=null, $store_id = 0 ){
		$childs = $this->getChild( $id, $store_id );
		foreach($childs as $child ){
			$this->children[$child['parent_id']][] = $child;	
		}
		$parent = 1 ;
		$output = $this->genTree( $parent, 1, $store_id );
		return $output;
	}
	...
	public function genTree( $parent, $level, $store_id = 0 ){
		if( $this->hasChild($parent) ){
			...
			foreach( $data as $menu ){
				$url  = $this->url->link('module/pavmegamenu', 'id='.$menu['megamenu_id'].$store.'&token=' . $this->session->data['token'], 'SSL') ;
				...
				if($menu['megamenu_id'] > 1) {
					$output .= $this->genTree( $menu['megamenu_id'], $level+1, $store_id );
				}
				$output .= '</li>';
			}
			$output .= '</ol>';
	...
	public function checkExitItemMenu($category){
		$query = $this->db->query("SELECT megamenu_id FROM ".DB_PREFIX."megamenu WHERE `type`='category' AND item=".$category['category_id']);
		if($query->num_rows){
			return false;
		}
		return true;
	...
	public function importCategories(){
		...
		foreach ($categories as &$category){
			...
			if($this->checkExitItemMenu($category)){
	...
	public function insertCategory($category = array()){
		...
		if($category['parent_id'] > 0){
			$query1 = $this->db->query("SELECT megamenu_id FROM ".DB_PREFIX."megamenu WHERE `type`='category' AND item=".$category['parent_id']);
			if($query1->num_rows){
				$data['megamenu']['parent_id'] = (int)$query1->row['megamenu_id'];
			}
		}
		...
		foreach( $data["megamenu"] as $key => $value ){
	 	...
	 	if( isset($category["language"]) ){
	 		...
	 		$this->db->query( $sql );
	 		$i = 0;
	 		foreach( $category["language"] as $categorydes ){
	 			$i++;
	 			$sql = "INSERT INTO ".DB_PREFIX ."megamenu_description(`language_id`, `megamenu_id`,`title`,`description`)
							VALUES(".$i.",'".$data['megamenu']['megamenu_id']."','".$this->db->escape($categorydes['name'])."','"
	 										.$this->db->escape($categorydes["description"])."') ";
	 			$this->db->query( $sql );
	 		}
	 	}
	}
