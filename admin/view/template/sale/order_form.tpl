  ...
  <div class="box">
    ...
    <div class="content" >
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        ...
        <div id="tab-customer" class="vtabs-content" style="width:49%;float:left">
          <table class="form">
			<tr><td><?php echo '№ заказа:' ?></td><td>#<?php echo $order_id; ?></td></tr>
			<tr>
			...
			<tr>
			 ...
			 <tr>
				  ...
				  <select name="shipping_code">
                    <option value="multiflat.multiflat0">Новая Почта</option>
				  </select>
*/ ?>
                      <select name="shipping_code">
                      ...
                      <input type="hidden" name="shipping_method" value="<?php echo 'Новая Почта';/*$shipping_method;*/ ?>" />
              ...
              <tr>
                  <td class="left">Адрес доставки для пункта Курьером в Киеве:</td>
                  <td class="left">
			...
			<tr>
                ...
                <td class="left">
					<select name="payment_code">
                        ...
                        <option value="cod"<? if ($payment_code == 'cod') echo ' selected="selected"'; ?>>Наличный расчет оплата при получении</option>
						<option value="bank_transfer"<? if ($payment_code == 'bank_transfer') echo ' selected="selected"'; ?>>Перевод на карту Приватбанка</option>
                        <option value="cheque"<? if ($payment_code == 'cheque') echo ' selected="selected"'; ?>>Безналичный платеж</option>

					</select>
                  <input type="hidden" name="payment_method" value="<?php echo htmlspecialchars($payment_method, ENT_COMPAT, 'utf-8'); ?>" />
                  </td>
            </tr>
			...
			<tr>
                ...
                <td class="left"><textarea name="order_comment" cols="40" rows="5"><?php echo $comment; ?></textarea></td>
              <td class="left"><a id="button-update" class="button">Обновить заказ</a></td>
			  </tr>	
          </table>
			  ...
			  <table class="list">
				<thead>
				  <tr>
					...
					<td class="right" style="white-space:nowrap"><?php echo $column_quantity; ?></td>
				...
				<tbody id="product">
				  ...
				  <tr id="product-row<?php echo $product_row; ?>">
					...
					<td class="left"><?php echo $order_product['name']; ?><br />
					  ...
					  <?php } ?></td>
					<td class="left"><?php 
						echo $order_product['product_id'] ? $order_product['model'] : ''; 
					?>
					  <input type="hidden" name="order_product[<?php echo $product_row; ?>][model]" value="<?php echo $order_product['model']; ?>" /></td>
					<td class="left"><?php 
						echo $order_product['product_id'] ? $order_product['sku'] : $order_product['model']; 
					?>
					  <input type="hidden" name="order_product[<?php echo $product_row; ?>][sku]" value="<?php echo $order_product['sku']; ?>" /></td>
					...
					<td class="right nowrap"><?php echo number_format($order_product['price'], 2, '.', '').''; ?>
					  <input type="hidden" name="order_product[<?php echo $product_row; ?>][price]" value="<?php echo $order_product['price']; ?>" /></td>
					<td class="right nowrap"><?php echo number_format($order_product['total'], 2, '.', '').''; ?>
				 ...
				 <tbody id="total">
				  ...
				  <tr id="total-row<?php echo $total_row; ?>">
					...
					<td class="right nowrap"><?php echo number_format($order_total['value'], 2, '.', '').''; ?></td>
				  </tr>
				  <?php $total_row++; ?>
				  <?php } ?>
			  ...
			  <table class="list">
				...
				<tbody>
				  ...
				  <tr>
					...
					<td class="left"><input type="text" name="quantity" value="1" /> &nbsp; <a id="button-product" class="button"><?php echo $button_add_product; ?></a></td>
				  </tr>             
				</tbody>
				<thead>
				  <tr>
					<td colspan="2" class="left"><?php echo 'Добавить от поставщика'; ?></td>
				  </tr>
				</thead>
				<tbody>
				  <tr><td class="left"><?php echo 'Название'; ?></td><td class="left"><input type="text" name="text_product" value="" /></td></tr>
				  <tr><td class="left"><?php echo 'Артикул'; ?></td><td class="left"><input type="text" name="text_sku" value="" /></td></tr>
				  <tr><td class="left"><?php echo 'Цена'; ?></td><td class="left"><input type="text" name="text_price" value="" /></td></tr>
				  <tr><td class="left"><?php echo 'Количество'; ?></td><td class="left"><input type="text" name="text_quantity" value="1" /> &nbsp; <a id="button-text-product" class="button"><?php echo $button_add_product; ?></a></td></tr>
				</tbody>
			  </table>
			</div>		  
		...
		<div id="tab-total" class="vtabs-content" style="width:49%;float:right">
		<table class="form tiny">
          <tr><td><?php echo 'Дата добавления:'; ?></td><td><?php echo $date_added; ?></td></tr>
		   <?php if ($order_status) { ?><tr><td><?php echo 'Статус заказа:'; ?></td><td id="order-status"><?php echo $order_status; ?></td></tr><?php } ?>
...
<div class="statuses"><?php echo $statuses; ?></div>
		  ...
		  <div id="tab-history" class="vtabs-content--">
			...
			<table class="form">
			  ...
			  <tr>
				<td><?php echo 'Уведомить покупателя:'; ?></td>
			  ...
			  <tr>
				...
				<? 
				 ...
				 <?/* <script type="text/javascript">
				  ...
				  <? } ?>
				  <div style="margin-top: 10px; text-align: right;"><a id="button-history" class="button"><?php echo 'Добавить историю'; ?></a></div></td>
			  </tr>
			</table>
		  </div>		
			...
			if (json['order_product'] != '' || json['order_voucher'] != '' || json['order_total'] != '') {
				...
				/*if (json['order_product'] != '') {
					for (i = 0; i < json['order_product'].length; i++) {
						...
						//html += '  <td class="left">' + product['sku'] + '</td>';
						// echo $order_product['product_id'] ? $order_product['sku'] : $order_product['model']; 
				...
				if (json['order_voucher'] != '') {
					for (i in json['order_voucher']) {
						...
						html += '  <td class="left" colspan="3"></td>';
						html += '  <td class="right">1</td>';
						html += '  <td class="right">' + voucher['amount'] + '</td>';
						html += '  <td class="right">' + voucher['amount'] + '</td>';
						html += '</tr>';	
