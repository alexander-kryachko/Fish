 ...
 <table class="ui-corner-all" cellpadding="0" cellspacing="0">
  <tfoot>
   <tr>
    ...
    <td class="center"><a onclick="$('.image_manager').remove();"><img style="border:none; float:none; vertical-align:middle;" src="view/batch_editor/image/close.png" title="" alt="" /></a>
    </td>
   </tr>
   <tr>
    <td class="left">
...
$(document).ready(function() {
	...
	$('#image_manager_keyword').catcomplete({
		source: function(request, response) {
			var data = '&directory=' + $('#image_manager_directory').val() + '&keyword=' + encodeURIComponent (request.term)
		...
		select: function(event, ui) {
			var value = $('#image_manager_directory').val() + ui.item.value;
