...
<div id="content">
  ...
  <div class="box">
    ...
    <div class="content">
	  ...
	  <a class="" style="display: inline;" href="#tab-productColor"><?php echo $tab_product_series; ?></a>
      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        <div id="tab-general">
          ...
          <div id="language<?php echo $language['language_id']; ?>">
            <table class="form">
            <tr>
              <td><a class="hidelink">Показать все поля</a></td>
              <td></td>
            </tr>		
          ...
          <table class="form">
            <tr>
              ...
              <td><?/*
				...
				// BOF: manufacturer autocomplete
				$manufacturers_array = array();
				$manufacturer_selected = false;
				foreach($manufacturers as $manufacturer){
					$manufacturers_array[] = '{value: "'.$manufacturer['manufacturer_id'].'", label: "'.str_replace('"', '\\"', $manufacturer['name']).'"}';
					if (isset($manufacturer_id) && $manufacturer_id == $manufacturer['manufacturer_id']) $manufacturer_selected = $manufacturer;
				}
				echo '<input type="hidden" name="manufacturer_id" value="'.($manufacturer_selected ? $manufacturer_selected['manufacturer_id'] : '0').'" />';
				echo '<input type="text" name="manufacturer_name" value="'.($manufacturer_selected ? $manufacturer_selected['name'] : '').'" style="width:350px" class="ui-autocomplete-input" autocomplete="off" role="textbox" aria-autocomplete="list" aria-haspopup="true" />';
				echo '<script type="text/javascript">var manufacturers = ['.implode(',', $manufacturers_array).']</script>';
				// EOF: manufacturer autocomplete
				?>
              ...
              <tr>
                  <td><span class="required">*</span> This product:</td>
                  <td>
                      <!--BOF Product Series -->
                      ...
                      <div>
                          ...
                          <table class="list" id="tbl_product_series_link">
                              <thead>
                              <tr>
                                  <td class="left">
                                      <?php echo $text_link_to_a_product_series; ?>
                                  </td>
                              </tr>
                              </thead>
                              <tbody>
							...
							*/
                                          // BOF: master_product autocomplete
                                          $master_products = array();
                                          $master_product_selected = false;
                                          foreach($all_master_product as $m_product){
                                              if($m_product['product_id'] != $product_id){
                                                  $master_products[] = '{value: "'.$m_product['product_id'].'", label: "'.str_replace('"', '\\"', $m_product['product_name']).'"}';
                                                  if (isset($master_product) && $master_product == $m_product['product_id']) $master_product_selected = $m_product;
                                              }
                                          }
                                          echo '<input type="hidden" name="master_product" value="'.($master_product_selected ? $master_product_selected['product_id'] : '').'" />';
                                          echo '<input type="text" name="master_product_name" value="'.($master_product_selected ? $master_product_selected['product_name'] : '').'" style="width:350px" class="ui-autocomplete-input" autocomplete="off" role="textbox" aria-autocomplete="list" aria-haspopup="true" />';
                                          echo '<script type="text/javascript">var master_products = ['.implode(',', $master_products).']</script>';
                                          // EOF: master_product autocomplete
                                          ?>
                                          ...
                                          <tr>
                                              ...
                                              <td class="right">
                                                  <?php if($l_product['product_id'] != $product_id) { ?>
                                                      [<a onclick="return confirm('<? echo $text_confirm_leave_page; ?>');" href="<?php echo $l_product['edit_href']; ?>" title="<?php echo $l_product['color_name']; ?>"><?php echo $text_edit; ?></a>]
                                                  <?php } ?>
                                              </td>
                                          </tr>
                                          </tbody>
            ...
            <tr>
              ...
              <td><div class="scrollbox cats">
					<?php $class = 'odd'; ?>
                  <?php foreach ($categories as $category) { ?>
                  <?php $class = ($class == 'even' ? 'odd' : 'even'); ?>

                  <div class="<?php echo $class; ?>">
                    <?php if (in_array($category['category_id'], $product_category_ids)) { ?>
...
-->
			...
			*/?>
			<input type="checkbox" name="product_store[]" value="0" checked="checked" style="visibility:hidden" />
...
-->			
		...
		<!-- RelatedLinks end -->
            ...
            <tr class="hide">
              ...
              <td><input type="text" name="mpn" maxlength="999" value="<?php echo $mpn; ?>" /></td>
            </tr>
            <tr class="hide">
        ...
        <div id="tab-attribute">
          <table id="attribute" class="list">
            ...
            <tfoot>
              <tr>
                <td <?php if ($this->config->get('pim_status') && !$this->config->get('pim_miu')) {?>colspan="3"<?php } else {?> colspan="2" <?php } ?>></td>
       ...
       <div id="vtab-option" class="vtabs">
		...
		<div id="tab-statuses"  class="hide">
...
//--></script> 
     ...
     <div id="tab-discount" class="hide">
          <table id="discount" class="list">
            <thead>
              <tr>
                ...
                <td class="right"><?php echo 'Цена, %:'; ?></td>
        ...
        <div id="tab-image">
          <table id="images" class="list">
            ...
            <tbody id="image-row<?php echo $image_row; ?>">
              <tr>
                <td class="left"><div class="image"><img src="<?php echo $product_image['thumb']; ?>" alt="" id="thumb<?php echo $image_row; ?>" />
                    ...
                    <br />
                    <a onclick="image_upload('image<?php echo $image_row; ?>', 'thumb<?php echo $image_row; ?>');"><?php echo $text_browse; ?></a>&nbsp;&nbsp;|&nbsp;&nbsp;<a onclick="$('#thumb<?php echo $image_row; ?>').attr('src', '<?php echo $no_image; ?>'); $('#image<?php echo $image_row; ?>').attr('value', '');$('#image_url<?php echo $image_row; ?>').val('');"><?php echo $text_clear; ?></a></div><br> Url image: <input type="text" name="product_image[<?php echo $image_row; ?>][image_url]" value="<?php echo $product_image['image']; ?>" id="image_url<?php echo $image_row; ?>" /></td>
            ...
            <tfoot>
              <tr>
                <td <?php if ($this->config->get('pim_status') && !$this->config->get('pim_miu')) {?>colspan="3"<?php } else {?> colspan="2" <?php } ?>></td>
                <td class="left"> <?php if ($this->config->get('pim_status')) {?><a onclick="image_upload();" class="button">Power Image Manager</a>&nbsp;<?php } ?><a onclick="addImage();" class="button"><?php echo $button_add_image; ?></a></td>
              </tr>
            </tfoot>
          </table>
        </div>
<!--        <div id="tab-reward">
        ...
        <div id="tab-design" class="hide">
          <table class="list">
			...
			<table class="list" id="tbl_product_color">
				<thead>
				<tr>
					<td class="left">
						<?php echo $column_product_series_image ?>
					</td>
				</tr>
				</thead>
				<tbody>
...
// BOF: autocomplete series and manufacturer
$('input[name=\'master_product_name\']').autocomplete({
	delay: 200,
	source: master_products,
	minLength: 2,
	select: function(event, ui) {
		$('input[name=\'master_product\']').val(ui.item.value ? ui.item.value : '');
		$('input[name=\'master_product_name\']').val(ui.item.label ? ui.item.label : '');
		return false;
	},
	focus: function(event, ui){
	  return false;
...
$('input[name=\'manufacturer_name\']').autocomplete({
	...
	minLength: 1,
	select: function(event, ui) {
		$('input[name=\'manufacturer_id\']').val(ui.item.value ? ui.item.value : '');
		$('input[name=\'manufacturer_name\']').val(ui.item.label ? ui.item.label : '');
		return false;
	},
	focus: function(event, ui){
	  return false;
...
// EOF: autocomplete series and manufacturer
...
$('input[name=\'product_category[]\']').live('change', function() {
    $('#tab-attribute table#attribute tbody').remove();
    var series_val = $('input[name=\'color_series_type\']:checked').val();

    var is_series = 0;
    if(series_val == 'singleItem' || series_val == 'colorSeriesMaster'){
        is_series = 1;
    }

    $('.cats > div').each(function(){
        var check = $(this).find('input[name=\'product_category[]\']');
        if(check.attr('checked') == 'checked'){
            var category_id = $(this).find('input[name=\'product_category[]\']').val();
            getCategoryAttributes(category_id, is_series);
        }

    })
});
function getCategoryAttributes(category_id, is_series = 0) {

  $.ajax({
            url: 'index.php?route=module/attribute_category/getAttributes&token=<?php echo $token; ?>&category_id=' +  category_id + '&series=' + is_series,
                dataType: 'json',
                success: function(json) {
...
function addImage() {
    ...
    html += '  <tr>';
    	html += '    <td class="left"><div class="image"><img src="<?php echo $no_image; ?>" alt="" id="thumb' + image_row + '" /><input type="hidden" name="product_image[' + image_row + '][image]" value="" id="image' + image_row + '" /><br /><a onclick="image_upload(\'image' + image_row + '\', \'thumb' + image_row + '\');"><?php echo $text_browse; ?></a>&nbsp;&nbsp;|&nbsp;&nbsp;<a onclick="$(\'#thumb' + image_row + '\').attr(\'src\', \'<?php echo $no_image; ?>\'); $(\'#image' + image_row + '\').attr(\'value\', \'\');$(\'#image_url'+image_row + '\').val(\'\');"><?php echo $text_clear; ?></a></div><br>Url: <input type="text" name="product_image[' + image_row + '][image_url]" value="" id="image_url' + image_row + '" /></td>';
...
<script type="text/javascript"><!--
 $(document).ready(function(){
    ...
    $("a.hidelink").click(function () {
	  var html = $("a.hidelink").html();
	  if (html == 'Показать все поля') $("a.hidelink").html('Скрыть дополнительные поля');
	  else $("a.hidelink").html('Показать все поля');
      $(".hide").toggle("slow");
    });
  });
//--></script>
