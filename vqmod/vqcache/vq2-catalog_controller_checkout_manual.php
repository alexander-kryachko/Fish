	...
	public function index() {
		...
		if ($this->user->isLogged() && $this->user->hasPermission('modify', 'sale/order')) {	
			...
			switch($data['payment_code']){
				case 'cod': $data['payment_method'] = 'Наличный расчет оплата при получении'; break;
				case 'bank_transfer': $data['payment_method'] = 'Перевод на карту Приватбанка'; break;
				default: $data['payment_method'] = ''; break;
			}
			switch($data['shipping_code']){
				default: $data['shipping_method'] = 'Новая почта';
			}
			
			$order_id = (int)$_POST['order_id'];
			$couponCode = trim($_POST['coupon']);
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', payment_code = '" . $this->db->escape($data['payment_code']) . "', payment_firstname = '" . $this->db->escape($data['firstname']) . "', payment_lastname = '" . $this->db->escape($data['lastname']) . "', payment_address_1 = '" . $this->db->escape($data['payment_address_1']) . "', payment_city = '" . $this->db->escape($data['payment_city']) . "', payment_method = '" . $this->db->escape($data['payment_method']) . "', shipping_firstname = '" . $this->db->escape($data['firstname']) . "', shipping_lastname = '" . $this->db->escape($data['lastname']) . "', shipping_address_1 = '" . $this->db->escape($data['payment_address_1']) . "', shipping_city = '" . $this->db->escape($data['payment_city']) . "', shipping_method = '" . $this->db->escape($data['shipping_method']) . "', comment = '" . $this->db->escape($data['order_comment']) . "', date_modified = NOW() WHERE order_id = '" . $order_id . "'");
			
			
			/*
			$query = $this->db->query("SELECT coupon_id FROM " . DB_PREFIX . "coupon_history WHERE order_id = ".$order_id." LIMIT 1");
			$prevCouponId = !empty($query->row) ? $query->row['coupon_id'] : false;
			echo $order_id;
			die();
			if ($prevCouponId){
				$this->db->query("DELETE FROM " . DB_PREFIX . "coupon_history WHERE order_id='" . $order_id ."' AND coupon_id = '".(int) $coupon_id."' LIMIT 1");
			}
			*/
			...
			if (isset($this->request->post['order_product'])) {
				foreach ($this->request->post['order_product'] as $order_product) {
					if ($order_product['product_id']){
						...
						$text_product = $order_product['name'];
						$text_sku = $order_product['model'];
						$text_price = $order_product['price'];
						$text_quantity = $order_product['quantity'];
						if ($text_product && $text_price && $text_quantity){
							$this->cart->addText($text_product, $text_sku, $text_price, $text_quantity);
						}
					}
				}
			}
			...
			if (isset($this->request->post['product_id'])) {
				...
				if ($product_info) {
					...
					if (!isset($json['error']['product']['option'])) $this->cart->add($this->request->post['product_id'], $quantity, $option);
				}
			}

			// add text product
			$text_product = isset($this->request->post['text_product']) ? trim($this->request->post['text_product']) : false;
			$text_sku = isset($this->request->post['text_sku']) ? trim($this->request->post['text_sku']) : false;
			$text_price = isset($this->request->post['text_price']) ? trim($this->request->post['text_price']) : false;
			$text_quantity = isset($this->request->post['text_quantity']) ? trim($this->request->post['text_quantity']) : false;
			if ($text_product && $text_price && $text_quantity){
				$this->cart->addText($text_product, $text_sku, $text_price, $text_quantity);
			}
				...
				if (!isset($json['error']['shipping'])) {
					...
					if (!$json['shipping_method']) {
						...
						if (!isset($shipping[0]) || !isset($shipping[1]) || !isset($json['shipping_method'][$shipping[0]]['quote'][$shipping[1]])) {		
							...
							$this->session->data['shipping_method'] = 'asdasd';$this->request->post['shipping_method'];
						}				
					}					
				}
			}
