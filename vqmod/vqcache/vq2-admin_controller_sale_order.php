...
//		$data['skip_order_status_id'] = 5;
        $order_total = $this->model_sale_order->getTotalOrders($data);

        $results = $this->model_sale_order->getOrders($data);

		$bannedEmails = $bannedPhones = array();
		$query = $this->db->query("SELECT `email` FROM `". DB_PREFIX."customer_ban_email`");
		if (!empty($query->rows)) foreach($query->rows as $r) $bannedEmails[] = $r['email'];
		$query = $this->db->query("SELECT `phone` FROM `". DB_PREFIX."customer_ban_phone`");
		if (!empty($query->rows)) foreach($query->rows as $r) $bannedPhones[] = substr($r['phone'], -10);		
        ...
        foreach ($results as $result) {
			...
			$warning = false;
			if (!empty($result['email']) && in_array($result['email'], $bannedEmails)) $warning = true;
			if (!empty($result['telephone']) && in_array(substr(preg_replace('/\D/', '', $result['telephone']), -10), $bannedPhones)) $warning = true;
            $this->data['orders'][] = array(
        ...
        $this->data['column_action'] = $this->language->get('column_action');
    					...
    					/* Nova Poshta express invoice*/    	
    					$this->language->load('sale/novaposhta_ei');
				    	$this->data['button_ei_list'] = $this->language->get('button_ei_list');
				    	$this->data['ei_list'] = $this->url->link('sale/novaposhta_ei', 'token=' . $this->session->data['token'], 'SSL');
    					/* Nova Poshta express invoice*/
...
//        }
        ...
        if (isset($this->request->post['coupon'])) {
			...
			$query = $this->db->query("SELECT c.code FROM " . DB_PREFIX . "coupon c INNER JOIN ".DB_PREFIX."coupon_history ch on (c.coupon_id = ch.coupon_id) WHERE ch.order_id = ".$this->request->get['order_id']." LIMIT 1");
			$this->data['coupon'] = !empty($query->row) ? $query->row['code'] : false;
        } else {
...
//        if ((utf8_strlen($this->request->post['payment_address_1']) < 1) || (utf8_strlen($this->request->post['payment_address_1']) > 128)) {
//            $this->error['payment_address_1'] = $this->language->get('error_address_1');
//        }

//        if ((utf8_strlen($this->request->post['payment_city']) < 1) || (utf8_strlen($this->request->post['payment_city']) > 128)) {
...
//        }
        ...
        if ($country_info) {
            if ($country_info['postcode_required'] && (utf8_strlen($this->request->post['payment_postcode']) < 1) || (utf8_strlen($this->request->post['payment_postcode']) > 10)) {
...
//            }
    ...
    public function info() {
        ...
        if ($order_info) {
    					...
    					if ($order_info['shipping_code'] == 'novaposhta.novaposhta') {
    						$this->language->load('sale/novaposhta_ei');
				    		$this->data['button_create_ei'] = $this->language->get('button_create_ei');
				    		$this->data['novaposhta_ei'] = $this->url->link('sale/novaposhta_ei/getForm', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
    					}
    					/* Nova Poshta express invoice*/
...
$this->data['payment_code'] = $order_info['payment_code'];
	...
	$qp = $this->db->query("SELECT `image` FROM " . DB_PREFIX . "product WHERE product_id = " . (int)$product['product_id'] . " LIMIT 1");
				$picture = !empty($qp->row['image']) ? $qp->row['image'] : false;
                ...
                $this->data['products'][] = array(
				'statuses'    => $product['statuses']['category'],
				'stickers'    => $product['statuses']['category_stickers'], 
...
//alphasms end -->
				...
				if (!empty($this->request->post['addtoban'])){
					$order_info = $this->model_sale_order->getOrder($this->request->get['order_id']);
					$email = $order_info['email'];
					$phone = preg_replace('/\D/', '', $order_info['telephone']);
					if (!empty($email)){
						$this->load->model('sale/customer_ban_email');
						$this->model_sale_customer_ban_email->addCustomerBanEmail(array('email' => $email));
					}
					if (!empty($phone)){
						$this->load->model('sale/customer_ban_phone');
						$this->model_sale_customer_ban_phone->addCustomerBanPhone(array('phone' => $phone));
					}
				}
...
$this->template = 'sale/order_invoice_suppler.tpl';
        $this->response->setOutput($this->render());
    }
}
?>
