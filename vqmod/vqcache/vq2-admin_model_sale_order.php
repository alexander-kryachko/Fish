...
class ModelSaleOrder extends Model {
	...
	public function editOrder($order_id, $data) {
		...
		switch($data['payment_code']){
      	...
      	$this->db->query("UPDATE `" . DB_PREFIX . "order` SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', payment_firstname = '" . $this->db->escape($data['payment_firstname']) . "', payment_lastname = '" . $this->db->escape($data['payment_lastname']) . "', payment_company = '" . $this->db->escape($data['payment_company']) . "', payment_company_id = '" . $this->db->escape($data['payment_company_id']) . "', payment_tax_id = '" . $this->db->escape($data['payment_tax_id']) . "', payment_address_1 = '" . $this->db->escape($data['payment_address_1']) . "', payment_address_2 = '" . $this->db->escape($data['payment_address_2']) . "', payment_city = '" . $this->db->escape($data['payment_city']) . "', payment_postcode = '" . $this->db->escape($data['payment_postcode']) . "', payment_country = '" . $this->db->escape($payment_country) . "', payment_country_id = '" . (int)$data['payment_country_id'] . "', payment_zone = '" . $this->db->escape($payment_zone) . "', payment_zone_id = '" . (int)$data['payment_zone_id'] . "', payment_address_format = '" . $this->db->escape($payment_address_format) . "', payment_method = '" . $this->db->escape($data['payment_method']) . "', payment_code = '" . $this->db->escape($data['payment_code']) . "', shipping_firstname = '" . $this->db->escape($data['shipping_firstname']) . "', shipping_lastname = '" . $this->db->escape($data['shipping_lastname']) . "',  shipping_company = '" . $this->db->escape($data['shipping_company']) . "', shipping_address_1 = '" . $this->db->escape($data['shipping_address_1']) . "', shipping_address_2 = '" . $this->db->escape($data['shipping_address_2']) . "', shipping_city = '" . $this->db->escape($data['shipping_city']) . "', shipping_postcode = '" . $this->db->escape($data['shipping_postcode']) . "', shipping_country = '" . $this->db->escape($shipping_country) . "', shipping_country_id = '" . (int)$data['shipping_country_id'] . "', shipping_zone = '" . $this->db->escape($shipping_zone) . "', shipping_zone_id = '" . (int)$data['shipping_zone_id'] . "', shipping_address_format = '" . $this->db->escape($shipping_address_format) . "', shipping_method = '" . $this->db->escape($data['shipping_method']) . "', shipping_code = '" . $this->db->escape($data['shipping_code']) . "', comment = '" . $this->db->escape($data['order_comment']) . "', affiliate_id  = '" . (int)$data['affiliate_id'] . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
		//order_status_id = '" . (int)$data['order_status_id'] . "', 
		...
		if (isset($data['order_total'])){
      		foreach ($data['order_total'] as $order_total){
				if (!(float)$order_total['value']) continue;
				...
				if ($order_total['code'] == 'coupon' && !empty($data['coupon'])){
					$query = $this->db->query("SELECT coupon_id FROM " . DB_PREFIX . "coupon WHERE `code` = '".$this->db->escape($data['coupon'])."' LIMIT 1");
					$couponId = !empty($query->row) ? $query->row['coupon_id'] : false;
					$amount = (float)$order_total['value'];
					if ($couponId) $this->db->query("INSERT INTO " . DB_PREFIX . "coupon_history(coupon_id, order_id, amount, date_added) VALUES (".$this->db->escape($couponId).", ".(int)$order_id.", ".$amount.", NOW())");
				}
			}
			$total += $order_total['value'];
		}
...
//		$sql = "SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.total, o.email, o.telephone, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";
//
...
//			$sql .= " WHERE o.order_status_id = '" . (int)$data['filter_order_status_id'] . "'";
		...
		} elseif (!empty($data['skip_order_status_id'])) {
			//$sql .= " WHERE (order_status_id > '0' AND `order_status_id` != ".$data['skip_order_status_id'].")";
			$sql .= " WHERE `order_status_id` <> ".$data['skip_order_status_id'];
		} else {
			//$sql .= " WHERE o.order_status_id > '0'";
			$sql .= " WHERE true";
		}
...
//		}
	...
	public function request_mail($order_info) {
		...
		if($order_info['order_status_id'] == $this->config->get('shop_rating_request_status') && $send == true) {
			...
			$mail_sr = new Mail();
			$mail_sr->protocol = $this->config->get('config_mail_protocol');
			$mail_sr->parameter = $this->config->get('config_mail_parameter');
			$mail_sr->smtp_hostname = $this->config->get('config_mail_smtp_hostname');
			$mail_sr->smtp_username = $this->config->get('config_mail_smtp_username');
			$mail_sr->smtp_password = html_entity_decode($this->config->get('config_mail_smtp_password'), ENT_QUOTES, 'UTF-8');
			$mail_sr->smtp_port = $this->config->get('config_mail_smtp_port');
			$mail_sr->smtp_timeout = $this->config->get('config_mail_smtp_timeout');


			$mail_sr->setTo($order_info['email']);
			$mail_sr->setFrom($this->config->get('config_email'));

			$mail_sr->setSender(html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'));
			$mail_sr->setSubject($subject);
			$mail_sr->setHtml($message);
			$mail_sr->setText(html_entity_decode($message, ENT_QUOTES, 'UTF-8'));
			$mail_sr->send();
	...
	public function addOrderHistory($order_id, $data) {
		...
		if (empty($query->row['order_status_id'])){
			//confirm
			$order_product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'");
